...
class VI_WOOCOMMERCE_PHOTO_REVIEWS_Frontend_Single_Page {
	...
	public function overall_rating_and_filter_html() {
		...
		$product_link  = wc_clean( wp_unslash( $_SERVER['REQUEST_URI'] ) );
	...
	public static function get_enqueue_params() {
		...
		$woocommerce_photo_reviews_params = array(
			...
			'container'                  => self::$frontend_style == 1 ? '.wcpr-grid' : ( ( self::$settings->get_params( 'reviews_container' ) ) ? self::$settings->get_params( 'reviews_container' ) : '.commentlist' ),
