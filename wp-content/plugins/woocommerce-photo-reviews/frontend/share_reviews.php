...
class VI_WOOCOMMERCE_PHOTO_REVIEWS_Frontend_Share_Reviews {
	...
	public function __construct() {
		...
		self::$share_group = self::$settings->get_params( 'share_reviews' );
		if ( 'off' !== self::$settings->get_params( 'enable' ) && count( self::$share_group ) ) {
	...
	public static function get_products( $product_id ) {
		...
		foreach ( self::$share_group as $k => $product_ids ) {
			...
			if ( $wpml_post_translations && get_option( 'wcml_reviews_in_all_languages' ) ) {
				$current_ids = array_merge( array_values( $wpml_post_translations->get_element_translations( $product_id ) ), $current_ids );
			}
