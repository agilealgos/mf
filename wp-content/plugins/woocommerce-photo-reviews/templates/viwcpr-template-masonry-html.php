...
foreach ( $my_comments as $v ) {
	...
	if ( $product ) {
		...
		printf( '<div id="%scomment-%s" class="%swcpr-grid-item"><div class="%swcpr-content">', $prefix_class, $v->comment_ID, $prefix_class, $prefix_class );
            ...
            <div class="<?php echo esc_attr( $prefix_class ); ?>reviews-images-container">
						...
						foreach ( $img_post_ids as $img_post_ids_k => $img_post_id ) {
							if ( ! wc_is_valid_url( $img_post_id ) ) {
								...
								if ( $is_video ) {
									...
									printf( '<div class="%sreviews-images-wrap"><a data-image_index="%s" data-image_src="%s" data-image_caption="%s" href="%s"><img class="%sreviews-images" src="%s" alt="%s"></a></div>',
								...
								if ( ! in_array( 'image/' . strtolower( $file_type ), $settings->get_params( 'upload_allow_images' ) ) ) {
									if ( strpos( $img_post_id, '.mp4' ) || strpos( $img_post_id, '.webm' ) ) {
										...
										printf( '<div class="%sreviews-images-wrap"><a data-image_index="%s" href="%s"><img class="%sreviews-images" src="%s" alt="%s"></a></div>',
											esc_attr( $prefix_class ), esc_attr( $img_post_ids_k ), esc_attr( $img_post_id ),
											esc_attr( $prefix_class ), esc_url( $img_post_id ), esc_attr( $product_title )
										);
                                    } else {
				...
				if ( ! wc_is_valid_url( $first_ele ) ) {
					...
					if ( $caption_enable ) {
						...
						if ( $is_video ) {
							printf( '<video class="%sreviews-images reviews-videos" data-original_src="%s" src="%s"  width="%s" height="%s" controls>%s</video>',
								...
								esc_attr( $img_width ), esc_attr( $img_height ), esc_attr( $image_alt )
							);
						} else {
							printf( '<img class="%sreviews-images" data-original_src="%s" src="%s" alt="%s" title="%s" width="%s" height="%s">',
								esc_attr( $prefix_class ),
								esc_attr( $src ), esc_url( apply_filters( 'woocommerce_photo_reviews_masonry_thumbnail_main', $src, $first_ele ) ),
						...
						if ( $is_video ) {
							printf( '<div class="%sreviews-images-wrap-right"><video class="%sreviews-images reviews-videos" data-original_src="%s" src="%s" width="%s" height="%s" controls>%s</video></div>',
							...
							printf( '<div class="%sreviews-images-wrap-right"><img class="%sreviews-images" data-original_src="%s" src="%s" alt="%s" title="%s" width="%s" height="%s"></div>',
					...
					$file_type = end( $file_type );
					if ( ! in_array( 'image/' . strtolower( $file_type ), $settings->get_params( 'upload_allow_images' ) ) ) {
						if ( strpos( $first_ele, '.mp4' ) || strpos( $first_ele, '.webm' ) ) {
							printf( '<div class="%sreviews-images-wrap-right"><video class="%sreviews-images reviews-videos" src="%s" controls>%s</video></div>',
								esc_attr( $prefix_class ), esc_attr( $prefix_class ), esc_url( $first_ele ), esc_attr( $product_title )
							);
						} else {
							printf( '<div class="%sreviews-images-wrap-right"><iframe class="%sreviews-images reviews-videos" src="%s" frameborder="0" allowfullscreen></iframe></div>',
								esc_attr( $prefix_class ), esc_attr( $prefix_class ), esc_url( $first_ele )
							);
						}
					} else {
						printf( '<div class="%sreviews-images-wrap-right"><img class="%sreviews-images" src="%s" alt="%s"></div>',
							esc_attr( $prefix_class ), esc_attr( $prefix_class ), esc_url( $first_ele ), esc_attr( $product_title )
						);
					}
				}
				if ( count( $img_post_ids ) > 1 ) {
					printf( '<div class="%simages-qty">+%s</div>', esc_attr( $prefix_class ), count( $img_post_ids ) - 1 );
				}
				?>
            </div>
			<?php
                    ...
                    <div class="<?php echo esc_attr( trim( implode( ' ', $comment_author_class ) ) ) ?>">
						...
						if ( 'yes' === get_option( 'woocommerce_review_rating_verification_label' ) && 1 == get_comment_meta( $comment->comment_ID, 'verified', true ) ) {
							switch ( $settings->get_params( 'photo', 'verified' ) ) {
								case 'badge':
									printf( '<em class="woocommerce-review__verified verified woocommerce-photo-reviews-verified %s"></em>', $settings->get_params( 'photo', 'verified_badge' ) );
									break;
								case 'text':
									printf( '<em class="woocommerce-review__verified verified woocommerce-photo-reviews-verified">%s</em>', $settings->get_params( 'photo', 'verified_text' ) );
									break;
								default:
				...
				if ( is_array( $custom_fields ) && count( $custom_fields ) ) {
					...
					foreach ( $custom_fields as $custom_field ) {
                        ...
                        <div class="wcpr-review-custom-field">
                            <span class="wcpr-review-custom-field-name"><?php echo wp_kses_post( $custom_field_name ) ?></span>:
		...
		if ( $comment_content_length > $max_content_length ) {
			...
			$comment_content = sprintf( '<div class="%swcpr-review-content-short">%s<span class="%swcpr-read-more" title="%s">%s</span></div><div class="%swcpr-review-content-full">%s</div>',
				...
				esc_attr( $prefix_class ), apply_filters( 'woocommerce_photo_reviews_masonry_review_content', nl2br( $comment->comment_content ), $comment ) );
		} else {
			$comment_content = apply_filters( 'woocommerce_photo_reviews_masonry_review_content', nl2br( $comment->comment_content ), $comment );
		}
		?>
		...
		do_action( 'woocommerce_photo_reviews_' . $prefix . 'masonry_item_after_main_content', $comment, $product );
					...
					foreach ( $comment_children as $comment_child ) {
                        ...
                        <div class="wcpr-comment-child">
							...
							if ( $comment_content_length > $max_content_length ) {
								...
								$comment_content   = sprintf( '<div class="%swcpr-review-content-short">%s<span class="%swcpr-read-more" title="%s">%s</span></div><div class="%swcpr-review-content-full">%s</div>',
									esc_attr( $prefix_class ), wp_kses_post( $comment_content_t ), esc_attr( $prefix_class ), esc_html__( 'Read more', 'woocommerce-photo-reviews' ),
									esc_html__( '...More', 'woocommerce-photo-reviews' ), esc_attr( $prefix_class ), apply_filters( 'woocommerce_photo_reviews_masonry_review_content', nl2br( $comment_content ), $comment ) );
							}
							?>
