...
class VI_WOOCOMMERCE_PHOTO_REVIEWS_Admin_Export {
	...
	public function add_menu() {
		add_submenu_page(
			...
			esc_html__( 'Export Reviews', 'woocommerce-photo-reviews' ),
			esc_html__( 'Export Reviews', 'woocommerce-photo-reviews' ),
			$this->settings->get_setting_capability(),
			'wcpr_export_reviews', array(
				$this,
	...
	public function export() {
		...
		if ( isset( $_POST['_woocommerce_photo_reviews_nonce'] ) && wp_verify_nonce( villatheme_sanitize_fields( $_POST['_woocommerce_photo_reviews_nonce'] ), '_woocommerce_photo_reviews_action_nonce' ) ) {
			$filename       = 'woocommerce_photo_reviews_export_';
			$start          = sanitize_text_field( $_POST['start_date'] );
			$end            = sanitize_text_field( $_POST['end_date'] );
			$products       = isset( $_POST['products'] ) ? villatheme_sanitize_kses( $_POST['products'] ) : array();
			$categories     = isset( $_POST['categories'] ) ? villatheme_sanitize_kses( $_POST['categories'] ) : array();
			$ratings        = isset( $_POST['ratings'] ) ? villatheme_sanitize_kses( $_POST['ratings'] ) : array();
			$comment_status = isset( $_POST['comment_status'] ) ? villatheme_sanitize_kses( $_POST['comment_status'] ) : array();
			$args           = array(
