		...
		<div class="<?php echo $ced_child_wrapper_class; ?>" id="ced_rnx_exchange_request_form_wrapper">
				...
				<table class="shop_table order_details ced_rnx_product_table">
					...
					<tbody>
						...
						<tr>
							...
							<td class="ced_rnx_total_amount_wrap"><span id="ced_rnx_total_exchange_amount"><?php echo ced_rnx_format_price( 0 ); ?></span>
																													  <?php
																														if ( $in_tax == true ) {
																															?>
									<small class="tax_label"><?php _e( '(incl. tax)', 'woocommerce-refund-and-exchange' ); ?></small>
					...
					<tbody>
						...
						foreach ( $exchange_to_products as $key => $exchange_to_product ) {
							...
							<tr>
								...
								<td ><?php echo sprintf( '<input type="number" data-product-id="%s" min="1" value="%s" class="ced_rnx_exchange_to_product_qty form-control" name="ced_rnx_exchange_to_product_qty">', $exchange_to_product['id'], $exchange_to_product['qty'] ); ?><?php // echo $exchange_to_product['qty']; ?></td>
								<td >
								<?php
								echo ced_rnx_format_price( $ced_rnx_exchange_to_product_price );
						...
						?>
						<tr class="exchange_product_table_footer">
							<th colspan="3"><?php _e( 'Total Amount', 'woocommerce-refund-and-exchange' ); ?></th>
							<th colspan="2" id="ced_rnx_exchanged_total_show"> 
							<?php
							echo ced_rnx_format_price( $total_price );
							if ( $ced_rnx_tax_test == true ) {
								?>
											<small class="tax_label"><?php _e( '(incl. tax)', 'woocommerce-refund-and-exchange' ); ?></small>
										<?php
			...
			<div class="ced_rnx_note_tag_wrapper lower_wrapper mwb_rma_flex">
				<div class="mwb_rma__column left_lower_wrapper">
					...
					<p class="form-row form-row form-row-wide">	
						<input type="text" name="ced_rnx_exchange_request_subject" maxlength="50" id="ced_rnx_exchange_request_subject_text" class="input-text ced_rnx_exchange_request_subject" placeholder="<?php _e( 'Write your reason subject', 'woocommerce-refund-and-exchange' ); ?>">
					</p>
					<?php
					...
					if ( isset( $predefined_exchange_desc ) ) {
						if ( $predefined_exchange_desc == 'yes' ) {
							...
							<p class="form-row form-row form-row-wide">
								...
								<?php
								$placeholder = get_option( 'ced_rnx_exchange_placeholder_text', 'Reason for Exchange Request' );
								if ( $placeholder == '' ) {
									$placeholder = __( 'Reason for the Exchange Request:', 'woocommerce-refund-and-exchange' );
								}
								?>
								<textarea name="ced_rnx_exchange_request_reason" cols="40" style="height: 222px;" maxlength="1000" class="ced_rnx_exchange_request_reason form-control" placeholder="<?php echo $placeholder; ?>"><?php echo $reason; ?></textarea>
							</p>
							<?php
						} else {
							?>
