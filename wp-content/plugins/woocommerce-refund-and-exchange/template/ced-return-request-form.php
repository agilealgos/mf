...
if ( $allowed ) {
			...
			<div class="lower_wrapper mwb_rma_flex">
				<div class="mwb_rma__column left_lower_wrapper">
					...
					<div class="ced_rnx_note_tag_wrapper">
						...
						<p class="form-row form-row form-row-wide">
							<input type="text" name="ced_rnx_return_request_subject" maxlength="50" class="input-text ced_rnx_return_request_subject" id="ced_rnx_return_request_subject_text" placeholder="<?php _e( 'Write your reason subject', 'woocommerce-refund-and-exchange' ); ?>">
						</p>
						...
						if ( isset( $predefined_return_desc ) ) {
							if ( $predefined_return_desc == 'yes' ) {
								...
								<p class="form-row form-row form-row-wide">
									...
									?>
									<textarea name="ced_rnx_return_request_reason" cols="40" maxlength="1000" style="height: 222px;" class="ced_rnx_return_request_reason form-control" placeholder="<?php echo $placeholder; ?>"><?php echo $reason; ?></textarea>
								</p>
								<?php
							} else {
								?>
						...
						if ( isset( $ced_rnx_return_wallet_enable ) && $ced_rnx_return_wallet_enable == 'yes' && is_user_logged_in() ) {
							if ( isset( $ced_rnx_select_refund_method_enable ) && $ced_rnx_select_refund_method_enable == 'yes' ) {
								...
								<br><label>
									<b>
										<?php
										_e( 'Select Amount Refund Method:', 'woocommerce-refund-and-exchange' );
										?>
									</b>
								</label>
								<br>
						...
						<form action="" method="post" id="ced_rnx_return_request_form" data-orderid="<?php echo esc_html( $order_id ); ?>" enctype="multipart/form-data">
							...
							$attach_limit = get_option( 'ced_rnx_return_attach_limit', '15' );
							...
							if ( isset( $return_attachment ) && ! empty( $return_attachment ) ) {
								if ( $return_attachment == 'yes' ) {
									...
									<p class="form-row form-row form-row-wide">
										...
										<input type="button" value="<?php _e( 'Add More', 'woocommerce-refund-and-exchange' ); ?>" class="btn button ced_rnx_return_request_morefiles" data-count="1" data-max="<?php echo $attach_limit; ?>">
	...
	$return_request_not_send = __( 'Refund Request can\'t be sent. ', 'woocommerce-refund-and-exchange' );

	$ced_main_wrapper_class = get_option( 'ced_rnx_return_exchange_class' );
	echo '<div class="woocommerce woocommerce-account '.$ced_main_wrapper_class.'">';
