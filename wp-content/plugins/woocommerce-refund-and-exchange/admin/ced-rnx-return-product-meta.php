	...
	foreach ( $return_datas as $key => $return_data ) {
		...
		if ( $return_data['status'] == 'complete' ) {
			...
			if ( $refundable_amount > 0 ) {
				_e( 'Following product refund request is approved on', 'woocommerce-refund-and-exchange' );
				?>
				 <b><?php echo $approve_date; ?>.</b><input type="button" name="ced_rnx_left_amount" class="button button-primary" data-orderid="<?php echo $order_id; ?>" id="ced_rnx_refund_left_amount" Value="Refund Amount" > 
				<?php
			} else {
				_e( 'Following product refund request is approved on', 'woocommerce-refund-and-exchange' );
				?>
		...
		if ( $return_data['status'] == 'cancel' ) {
			...
			_e( 'Following product refund request is canceled on', 'woocommerce-refund-and-exchange' );
			?>
			 <b><?php echo $approve_date; ?>.</b>
			<?php
		}
