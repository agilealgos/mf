	...
	class CED_rnx_order_return {
		...
		function ced_rnx_return_product_info_callback() {
			...
			if ( $check_ajax ) {
				...
				if ( 'yes' === $wallet_enable && 'yes' !== $ced_rnx_refund_method ) {
				...
				if ( $day_allowed >= $day_diff && $day_allowed != 0 && 'yes' === $auto_re ) {
				...
				if ( isset( $_FILES['ced_rnx_return_request_files']['tmp_name'] ) ) {
					...
					for ( $i = 0;$i < $count;$i++ ) {
						if ( isset( $_FILES['ced_rnx_return_request_files']['tmp_name'][ $i ] ) ) {
							...
							$file_type  = $_FILES['ced_rnx_return_request_files']['type'][ $i ];
		...
		function ced_rnx_order_return_button( $order ) {
				...
				if ( isset( $product_datas ) && ! empty( $product_datas ) ) {
					...
					foreach ( $product_datas as $key => $product_data ) {
						...
						if ( in_array( $order_status, $statuses ) ) {
							if ( $product_data['status'] == 'pending' ) {
								...
								if ( isset( $ced_rnx_catalog_refund_days ) && $ced_rnx_catalog_refund_days != 0 && 'yes' != $refund_button_hide ) {
									...
									if ( $day_allowed >= $day_diff && $day_allowed != 0 && 'yes' != $refund_button_hide ) {
										if ( $ced_rnx_return_button_show ) {
											$ced_rnx_return_button_show = false;
											$ced_rnx_pages = get_option( 'ced_rnx_pages' );
											$page_id = $ced_rnx_pages['pages']['ced_return_from'];
						...
						if ( $product_data['status'] == 'cancel' ) {
							...
							$appdate = date_format( $appdate, $format );
							?>
							<p><?php _e( 'Above product Refund request is canceled on', 'woocommerce-refund-and-exchange' ); ?> <b><?php echo $appdate; ?>.</b></p>
							<?php
						}
					}
			...
			if ( isset( $cancel_product_enable ) && $cancel_enable == 'yes' && $cancel_product_enable == 'yes' && $ced_rnx_cancel_button_show == true ) {
				if ( in_array( $order_status, $cancel_order_status ) ) {
					...
					<form action="<?php echo esc_html( add_query_arg( 'order_id', $order_id, $cancel_form_url ) ); ?>" method="post">
						<input type="hidden" value="<?php echo esc_html( $order_id ); ?>" name="order_id">
						<p><input type="submit" class="btn button" value="<?php _e( $cancel_button_text, 'woocommerce-refund-and-exchange' ); ?>" name="ced_rnx_new_return_request"></p>
					</form>
					<?php
				}
			}
