...
class MiniCart {
	...
	function add_cart_into_header(){
		...
		reyCoreAssets()->add_styles('rey-wc-header-mini-cart-top', 'rey-wc-header-mini-cart', 'reycore-side-panel');
		reyCoreAssets()->add_scripts('reycore-wc-header-minicart');
	...
	function add_cart_panel(){
		...
		reyCoreAssets()->add_styles(['rey-wc-general', 'rey-wc-header-mini-cart-top']);
	...
	public function after_mini_cart() {
		...
		if( wp_doing_ajax() ){
			...
			if( empty($this->collected_assets) ){
				$this->collected_assets = reyCoreAssets()->collect_end(true);
			}
	...
	public static function load_panel_assets(){
		...
		reyCoreAssets()->add_styles(['rey-simple-scrollbar', 'rey-wc-header-mini-cart', 'reycore-side-panel']);
		reyCoreAssets()->add_scripts( ['rey-simple-scrollbar', 'reycore-wc-product-page-qty-controls'] );
	...
	function show_shipping_minicart()
	{
		...
		$cost_html = apply_filters('reycore/minicart/shipping/cost_html', '', $cost, $this);
		...
		printf(
			'<div class="minicart-total-row"><div class="minicart-total-row-head">%1$s</div><div class="minicart-total-row-content">%2$s</div></div>',
	 ...
	 * @return array
	 */
	public function prepare_products_data_fragment( $products = [], $type = 'cross_sells' ){
	...
	public function get_clean_atc_button(){
		...
		remove_filter('woocommerce_product_add_to_cart_text', [reycore_wc__get_tag('cart'), 'cross_sells_buttons_text'], 20);
