					...
					<# for ( choiceKey in data.default ) { #>
						<div class="{{ choiceKey }}">
							...
							<div class="{{ choiceKey }} input-wrapper">
								...
								<# var attributes = data.inputAttrs;
								if( data.inputAttrs.indexOf('{') === 0 ){
									var objectAttributes = JSON.parse(data.inputAttrs);
									attributes = '';
									for ( attrKey in objectAttributes ) {
										if( typeof objectAttributes[attrKey] === 'object' ){
											if( choiceKey === attrKey ){
												for ( attrKeyInner in objectAttributes[attrKey] ) {
													attributes += ' '+ attrKeyInner +'="'+ objectAttributes[attrKey][attrKeyInner] +'"';
												}
											}
										}
										else {
											attributes += ' '+ attrKey +'="'+ objectAttributes[attrKey] +'"';
										}
									}
								} #>
								<input {{{ attributes }}} type="text" data-choice="{{ choiceKey }}" value="{{ val }}"/>
							</div>
						</div>
					<# } #>
				</div>
