...
class Kirki_Control_Base extends WP_Customize_Control {
	...
	public function to_json() {
		...
		// Input attributes.
		$this->json['inputAttrs'] = $this->getInputAttrs();
		...
		$this->json['label-tooltip'] = $this->label_tooltip;
	...
	public function getInputAttrs(){
		...
		// flag when there's an array inside, for passing inputAttrs as object
		$has_array_inside = count(array_filter($this->input_attrs, function($val){
			return is_array($val);
		}));

		if( $has_array_inside ){
			$inputAttrs = wp_json_encode($this->input_attrs);
		}
		// just pass as string
		else {
			foreach ($this->input_attrs as $attr => $value) {
				$inputAttrs .= sprintf('%s="%s" ', $attr, esc_attr( $value ) );
			}
		}
