...
class QueryControl
{
	...
	function get_autocomplete_for_terms( $data ){
		...
		if( in_array($data['query_args']['taxonomy'], ['all_taxonomies', 'product_taxonomies'], true) ){
			...
			foreach ( $terms as $term ) {
				...
				$results[] = [
					'id' 	=> $term->$key,
					'text' 	=> sprintf($term->name . ' (%s)', $tax_label),
				];
			}
	...
	public function elementor_ajax_list( $data ){
		...
		$req = reycore__clean($data['request'], false);

		if( is_callable( $req ) ){
			$results = call_user_func( $req, $data );
		}
	...
	public function register_ajax_actions( $ajax_manager ) {
		...
		$ajax_manager->register_ajax_action( 'rey_ajax_list_control', [ $this, 'elementor_ajax_list' ] );
	}
