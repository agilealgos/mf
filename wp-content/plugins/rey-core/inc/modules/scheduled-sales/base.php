...
class Base extends \ReyCore\Modules\ModuleBase {
	...
	public function render(){
		...
		if(
			...
			isset($evergreen['starting_from'], $evergreen['duration']) &&
			($duration = absint($evergreen['duration'])) &&
			($starting_date = $evergreen['starting_from'])
		){
			$wp_timezone = wp_timezone();
			$initial_start_date = new \DateTime( $starting_date, $wp_timezone );
			$days_since_initial_start_and_now = absint($now->diff($initial_start_date)->format("%a"));
			...
			// $latest_cycle_started = new \DateTime( sprintf( '%s +%d day', $starting_date, $days_since_initial_start_and_latest_cycle_start) , $wp_timezone );

			// stop if exceeded the repetitions
			if( isset( $evergreen['repeat_count'] ) && ($repeat_count = absint($evergreen['repeat_count'])) && ($repeat_count < $past_repeated_cycles) ){
				return;
			}
			...
			$sale_to = new \DateTime( sprintf( '%s +%d day', $starting_date, $sale_to_days ) , $wp_timezone );
		...
		if( $bg = $this->data[$env]['bg'] ){
			$args['classes'][] = '--bg';
		}

		$type = $this->data[$env]['type'];
		$position = $this->data[ $env ]['position'];
		...
		elseif( $type === 'countdown' ){
			...
			$output = $this->render_countdown($args);
		...
		if( $output ){
			...
			if( $this->data[$env]['stretch'] ){
				$classes[] = '--stretch';
			}

			if( $this->data[$env]['center'] ){
				$classes[] = '--center';
			}
	...
	function render_badge( $args ){
		...
		if( $custom_text = $this->data[ $this->get_env() ]['text'] ){
	...
	function render_countdown( $args ){
		...
		foreach(['d', 'h', 'i', 's'] as $d){
			...
			if( isset($args['inline']) && $args['inline'] ){
				...
				$item_text = substr($text_strings[$d]['singular'], 0, 1);
			...
			// regular
				...
				$item_text = $text_strings[$d]['plural'];

				if( $this->defaults['countdown_use_short'] && isset($text_strings[$d]['abbr']) ){
					$item_text = $text_strings[$d]['abbr'];
				}

			}

			$text .= sprintf(' <li class="--%1$s"><span class="__item" data-val="%2$s">%2$s</span><span class="__text">%3$s</span></li>',
				$d,
				$remaining->{$d},
				$item_text
			);
		...
		if( isset($args['inline']) && $args['inline'] ){
			...
			$badge_icon = $this->defaults['badge_use_icon'] ? $this->defaults['badge_icon'] : '';
			// add output
			$before .= sprintf('<li class="__icon">%s</li>', $badge_icon);
		}

		if( $custom_text = $this->data[ $env ]['text'] ){
			$before .= sprintf('<li class="__title">%s</li>', $custom_text);
		}

		self::load_scripts(true);

		return sprintf('<ul class="rey-countDown %s" data-finish="%s">%s</ul>',
			esc_attr(implode(' ', $args['classes'])),
			esc_attr( $args['to']->format('Y/m/d H:i:s P') ),
			$before . $text
		);
	}
	 ...
	 * @return void
	 */
	public static function load_scripts($with_js = false){

		if( self::$scrips_loaded ){
			return;
		}

		reyCoreAssets()->add_styles(Base::ASSET_HANDLE);

		if( $with_js ){
			reyCoreAssets()->add_scripts(Base::ASSET_HANDLE);
		}
