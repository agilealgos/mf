...
class Admin
{
	...
	public function __construct()
	{
		...
		add_filter( 'rey/export/page_options', [$this, 'export_swatches_options']);
	...
	public function demo_import( $config ){
		...
		if( isset($config[ Base::OPT ]) && ! empty($config[ Base::OPT ])){
			update_option(Base::OPT, $config[ Base::OPT ]);
		}
	...
	function customizer__add_controls($control_args, $section){
		...
		$section->add_control( [
			...
			'default'     => 'dim',
			'choices'     => [
				'dim'   => esc_html__( 'Dim swatch', 'rey-core' ),
				'xmark' => esc_html__( 'Show X Mark', 'rey-core' ),
