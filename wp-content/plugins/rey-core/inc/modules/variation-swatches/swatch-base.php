...
class SwatchBase
{
	...
	function render_list( $args = [] ){
		...
		$html_list_attributes['data-attribute_name'] = 'attribute_' . sanitize_title($this->list_args['taxonomy']);
	...
	function default_item_html_attributes($term){
		...
		$html_attributes['data-attribute-name'] = sanitize_title($this->list_args['taxonomy']);
	...
	function render_item(){
		...
		foreach ( $terms as $i => $term ) {
			...
			$classes[] = $this->list_args['selected'] === $term->slug ? '--selected' : '';
	...
	function render_tooltip($term){
		...
		if( isset($this->attribute_settings['swatch_tooltip']) && 'yes_desc' === $this->attribute_settings['swatch_tooltip'] && ($desc = $term->description) ){
