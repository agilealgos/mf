...
class Element extends \ReyCore\Modules\CustomTemplates\WooBase {
	...
	public function get_name() {
		return 'reycore-woo-pdp-wishlist';
	}

	public function get_title() {
		return __( 'Wishlist Button (PDP)', 'rey-core' );
	}
	...
	protected function element_register_controls() {
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				[
					...
					'raw' => sprintf( _x( '<a href="%s" target="_blank" class="__title-link">Customize Wishlist options<i class="eicon-editor-external-link"></i></a><br>Access Customizer > WooCommerce > Wishlist to customize the display of this feature.', 'Elementor control label', 'rey-core' ), add_query_arg( ['autofocus[panel]' => 'wishlist'], admin_url( 'customize.php' ) ) ),
				]
			);
		...
		$this->start_controls_section(
			...
			$selectors = [
				'main' => '{{WRAPPER}} .rey-wishlistBtn-wrapper',
				'btn' => '{{WRAPPER}} .rey-wishlistBtn',
				'btn_hover' => '{{WRAPPER}} .rey-wishlistBtn:hover',
			];
	...
	function render_template() {
		...
		add_filter('theme_mod_wishlist_pdp__enable', '__return_true');

		add_filter('theme_mod_wishlist_pdp__btn_style', [$this, 'btn_style']);
		add_filter('theme_mod_wishlist_pdp__wtext', [$this, 'text_visibility']);
		add_filter('theme_mod_wishlist_pdp__tooltip', [$this, 'show_tooltip']);

		Base::instance()->output_pdp_button();

		remove_filter('theme_mod_wishlist_pdp__btn_style', [$this, 'btn_style']);
		remove_filter('theme_mod_wishlist_pdp__wtext', [$this, 'text_visibility']);
		remove_filter('theme_mod_wishlist_pdp__tooltip', [$this, 'show_tooltip']);
	}
