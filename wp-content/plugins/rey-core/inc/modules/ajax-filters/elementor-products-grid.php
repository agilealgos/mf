...
class ElementorProductsGrid
{
	...
	public function override_product_grid_query($query_vars, $id, $settings )
	{
		...
		if( ! $base->get_chosen_filters() ){
			$base->set_chosen_filters();
		}

		// if nothing, just bail
		if( ! ( $chosen_filters = $base->get_chosen_filters() ) ){
			return $query_vars;
		}

		if( ! empty($chosen_filters) && isset($chosen_filters['chosen'] ) ){

			$vars_from_url = array_filter([
				'meta_query'   => $base->query_for_meta(),
				'tax_query'    => $base->query_for_tax(),
				'post__in'     => $base->query_for_post__in(),
				'post__not_in' => $base->query_for_post__not_in()
			]);
			...
			if(
				isset($chosen_filters['active_filters']['orderby']) &&
				$orderby = $chosen_filters['active_filters']['orderby']
			){
	...
	public function fix_sale_counters( $query ){
		...
		$on_sale_products = Base::instance()->onsale_products([
	...
	public function sidebar_add_scripts(){
		...
		if( ! ( $base->filter_widgets_exist() || $base->check_forced_link_parameters() ) ){
			return;
		}
	...
	public function adds_element_options( $element ){
		...
		$element->add_control(
			...
			[
				...
				'type' => 'rey-ajax-list',
				'query_args' => [
					'request' => [__CLASS__, 'get_button_sidebars'],
				],
			]
		);
	...
	public static function get_button_sidebars(){
