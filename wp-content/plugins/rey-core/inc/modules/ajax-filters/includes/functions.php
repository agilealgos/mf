...
if(!function_exists('reyajaxfilter_search_query')):
	...
	function reyajaxfilter_search_query($args = []){
		...
		if( (isset($_GET['on-sale']) && 1 === absint($_GET['on-sale'])) || (isset($args['onsale']) && $args['onsale']) ){
			if( $sale_ids = \ReyCore\Modules\AjaxFilters\Base::instance()->onsale_products() ){
...
if(!function_exists('reyajaxfilter_get_filtered_term_product_counts')):
	...
	function reyajaxfilter_get_filtered_term_product_counts( $term_ids, $taxonomy, $query_type ) {
		...
		// Woo 6.0+ has built-in attribute lookup tables and filtering became easier
		if(
			...
			get_theme_mod('ajaxfilters_attributes_lookup_table', true)
		){
			return wc_get_container()->get( 'Automattic\WooCommerce\Internal\ProductAttributesLookup\Filterer' )->get_filtered_term_product_counts( $term_ids, $taxonomy, $query_type );
		}
...
if (!function_exists('reyajaxfilter_clear_transients')) {
	function reyajaxfilter_clear_transients() {
		return \ReyCore\Helper::clean_db_transient( 'reyajaxfilter' );
	}
}
...
if (!function_exists('reyajaxfilter_terms_output')):
	...
	function reyajaxfilter_terms_output($args) {
		...
		$chosen_filters = \ReyCore\Modules\AjaxFilters\Base::instance()->get_chosen_filters();
		...
		if( $manual_ids = $args['manually_pick_ids'] ){
			...
			$parent_args['orderby'] = 'menu_order';
		}
		...
		if( isset($args['url_array']['on-sale']) && $args['url_array']['on-sale'] ){
			...
			$get_sale_terms = array_unique(wp_get_object_terms( \ReyCore\Modules\AjaxFilters\Base::instance()->onsale_products(), $args['taxonomy'], $parent_args ));
		...
		foreach ($args['sub_term_ids'] as $sub_term_id) {
			...
			if ( $show_term ) {
				...
				if( ! $args['dropdown'] ){
					...
					$link_attributes['aria_label'] = sprintf('aria-label="%s"', esc_attr($_term_name));
	...
	function reyajaxfilter_main_terms_output($args) {
		...
		foreach ($args['terms'] as $term_id) {
			...
			if ( $show_term ) {
				...
				$link_attributes['slug'] = sprintf('data-slug="%s"', esc_attr($term->slug));
				$link_attributes['jump'] = reyajaxfilter_jump_to_cat($term, $args);
				$link_attributes['aria_label'] = sprintf('aria-label="%s"', esc_attr($term->name));
		...
		if( ! $args['accordion_list'] && $custom_height = absint($args['custom_height'] ) ){
			...
			reyCoreAssets()->add_scripts('rey-simple-scrollbar');
...
if(!function_exists('reyajaxfilter_droppanel_output')):
	...
	function reyajaxfilter_droppanel_output( $html, $args = [] )
	{
		...
		reyCoreAssets()->add_scripts('reycore-ajaxfilter-droppanel');
		reyCoreAssets()->add_styles(['reycore-ajaxfilter-droppanel']);
...
if (!function_exists('reyajaxfilter_dropdown_terms')):
	function reyajaxfilter_dropdown_terms($args) {
		...
		if (!empty($args['terms'])) {
			...
			// required scripts
			reyCoreAssets()->add_scripts('reycore-ajaxfilter-select2');
			reyCoreAssets()->add_styles(['reycore-ajaxfilter-dropdown', 'reycore-ajaxfilter-select2']);
			...
			$html .= '<div class="reyajfilter-dropdown-nav">';
				...
				if( $args['show_checkboxes'] ):
					...
					if( $args['enable_multiple'] ) {
						reyCoreAssets()->add_scripts('reycore-ajaxfilter-select2-multi-checkboxes');
					}
...
if(!function_exists('reyajaxfilter_show_active_tax')):
	/**
	 ...
	 * @since 2.0.5
	 **/
	function reyajaxfilter_show_active_tax( $filters )
	{

		// When in a taxonomy,
		// append the tax. name as active term.
		if( ! get_theme_mod('ajaxfilters__hide_active_tax_filter', false) ){
			return $filters;
		}

		if( ! (is_tax() && ($current_tax = get_queried_object())) ){
			return $filters;
		}
		...
		// custom taxonomies
		foreach ( \ReyCore\Modules\AjaxFilters\Base::instance()->get_registered_taxonomies() as $taxonomy ) {
		...
		foreach ($available_taxonomies as $tax_name => $url_param) {
			if( isset($current_tax->taxonomy) && $current_tax->taxonomy === $tax_name ) {
				$filters['chosen'][$tax_name]['terms'][] = $current_tax->term_id;
				$filters['chosen'][$tax_name]['query_type'] = 'or';
				$filters['active_filters']['term'][$url_param][ $current_tax->term_id ] = $tax_name;
			}
		}
	...
	add_filter('reycore/ajaxfilters/chosen_filters', 'reyajaxfilter_show_active_tax');
