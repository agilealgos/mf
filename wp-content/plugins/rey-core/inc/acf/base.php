...
class Base {
	...
	public function __construct(){
		...
		add_filter('acf/prepare_field/name=preload_google_fonts', [$this, 'preload_google_fonts'] );
		add_filter('acf/prepare_field/name=gs_type', [$this, 'gs_type_instructions'] );
		...
		add_filter('acf/prepare_field/name=perf__css_exclude', [$this, 'populate_css_excludes'] );
	 ...
	 * @since 1.0.0
	 */
	function preload_google_fonts( $field ) {
		...
		$url = add_query_arg([
			'url' => $customize_url,
			'autofocus[section]' => \ReyCore\Customizer\Options\General\Typography::get_id()
			], admin_url( 'customize.php' )
		);

		$field['instructions'] = sprintf(__('Pick one or two Google fonts you will load in your website. To actually choose them to be used, head over to <a href="%1$s" target="_blank">Customizer > General > Site Typography</a> and pick as Primary or Secondary font family. <br> This tool helps in preventing loading a webfont twice, by choosing a "single source of truth". For example in Customizer or Elementor, make sure to pick one of the preloaded fonts eg: <a href="%2$s" target="_blank">%2$s</a>. Visit <a href="%3$s" target="_blank">this article</a> for more info.', 'rey-core'), $url, 'https://d.pr/i/Cmcxi8', reycore__support_url('kb/how-to-properly-use-google-fonts/') );
	 ...
	 * @since 1.0.0
	 */
	function gs_type_instructions( $field ) {
	...
	/**
	 * Add exclude assets option
	 * @since 2.0.0
	 */
	function populate_css_excludes( $field ) {

		$styles = \ReyCore\Assets::get_excludes_choices( false );
