...
class Helper {
	...
	public static function get_props( $prop = '' ){
		...
		$props = \ReyCore\Plugin::instance()->elementor::$props;
	...
	public static function is_optimized_dom() {
		...
		if( self::get_props('elementor_major_version') < 3 ){
			return false;
		}

		return self::is_experiment_active('e_dom_optimization');
	...
	public static function el_inject_html( $content, $injection, $query )
	{
		...
		if ( ( class_exists( '\DOMDocument' ) && class_exists( '\DOMXPath' ) && function_exists( 'libxml_use_internal_errors' ) ) ) {
			...
			if( function_exists('mb_convert_encoding') ){
				...
				error_log('\ReyCore\Elementor: `mb_convert_encoding` function is not installed on your server and may cause display errors with UTF8 encoding. Please check phpinfo() make sure that the `mbstring` extension is loaded.');
