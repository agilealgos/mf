...
class Menu extends \ReyCore\Elementor\WidgetsBase {
				...
				$this->add_responsive_control(
					...
					[
						...
						'selectors' => [
							...
							'{{WRAPPER}} .reyEl-menu .menu-item > a:hover' => 'color: {{VALUE}}',
						],
					]
				);
	...
	public function render_title($settings){
		...
		$title = $inner['before'] . $title . $inner['after'] . reycore__get_svg_icon(['id'=>'arrow']);
		...
		echo '<div class="reyEl-menu-navWrapper">';
			...
			wp_nav_menu([
				...
				'menu_class' => implode(' ', [
					'reyEl-menu-nav',
					'rey-navEl',
					'--menuHover-' . $settings['hover_style'],
				]),
				'items_wrap'  => '<ul id="%1$s" class="%2$s">%3$s</ul>',
				'link_before' => '<span>',
				'link_after'  => '</span>',
				'depth'       => $settings['menu_depth'] ? $settings['menu_depth'] : 1,
				'walker'      => new \ReyCore\Libs\Nav_Walker,
				'cache_menu'  => $settings['menu_cache'] !== ''
			]);
