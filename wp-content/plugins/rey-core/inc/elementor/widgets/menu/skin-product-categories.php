...
class SkinProductCategories extends \Elementor\Skin_Base
{
	...
	public function render_all_categories(){
		...
		if( $this->_settings['hide_uncateg'] !== '' ){
			$list_args['exclude'] = $this->_settings['depth'];
		}
	...
	public function get_transient_name(){
		...
		return \ReyCore\Menus::CAT_TRANSIENT . md5( wp_json_encode($settings) );
	}
	...
	public function render() {
		...
		ob_start();
		$this->parent->render_start($this->_settings);
		$this->parent->render_title($this->_settings);
		$start = ob_get_clean();
