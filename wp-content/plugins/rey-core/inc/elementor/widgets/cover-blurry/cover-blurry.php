...
class CoverBlurry extends \ReyCore\Elementor\WidgetsBase {
	...
	public function rey_get_script_depends() {
		return [ 'rey-videos', 'rey-splide', 'reycore-widget-cover-blurry-scripts' ];
	}
	...
	public function render_start(){
		...
		if( ! reycore__preloader_is_active() && ! reycore__js_is_delayed() ){
			$classes[] = '--loading';
		}
		...
		<?php
			if( !empty($this->_items) ):
				foreach($this->_items as $key => $item):
					...
					if( $key === 0 && reycore__js_is_delayed() ){
						$classes[] = 'is-active';
					}
	...
	public function render_arrows()
	{
		if( $this->_settings['arrows'] === 'yes' ): ?>
			<div class="cBlurry-arrows __arrows-<?php echo $this->get_id() ?>">
				<?php
					reycore__svg_arrows([
	...
	public function render_nav()
	{
		if( !empty($this->_items) ){
			...
			for( $i = 0; $i < count($this->_items); $i++ ){
				$bullets .= sprintf( '<button data-go="%d"></button>', $i );
			}
	...
	protected function render() {
		...
		reyCoreAssets()->add_styles(['reycore-widget-cover-blurry-styles', 'rey-splide']);
