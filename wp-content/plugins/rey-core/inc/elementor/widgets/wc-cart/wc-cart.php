...
class WcCart extends \ReyCore\Elementor\WidgetsBase {
	...
	public function rey_get_script_depends() {
		return [ 'wp-util', 'reycore-woocommerce', 'reycore-widget-wc-cart-scripts' ];
	}
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
			...
			$this->add_responsive_control(
				'process_distance',
				[
					'label' => esc_html__( 'Bottom Distance', 'rey-core' ) . ' (px)',
					'type' => \Elementor\Controls_Manager::SLIDER,
					'range' => [
						'px' => [
							'min' => 0,
	...
	public function render_main(){
		...
		reyCoreAssets()->add_styles('rey-wc-cart');
	...
	public function render_start(){
		...
		if($this->_settings['totals__sticky'] === 'yes' ){
			...
			if( $sticky_offset = $this->_settings['totals__sticky_offset'] ){
	...
	protected function render() {
		...
		reyCoreAssets()->add_styles(['reycore-widget-wc-cart-styles']);
