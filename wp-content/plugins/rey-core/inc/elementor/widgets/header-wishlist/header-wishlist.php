...
class HeaderWishlist extends \ReyCore\Elementor\WidgetsBase {
	...
	{
		reyCoreAssets()->add_styles(['rey-header-drop-panel', 'rey-wc-header-wishlist']);
	...
	public function render_button(){
		...
		if( $this->_settings['counter_layout'] !== '' ){
			add_filter('theme_mod_header_account_wishlist_counter', '__return_true');
			ob_start();
			reycore__get_template_part('template-parts/woocommerce/header-account-wishlist-count');
			$counter_layout_html = ob_get_clean();
			remove_filter('theme_mod_header_account_wishlist_counter', '__return_true');
		}
	...
	public function render_panel() {
		...
		echo '<div class="rey-header-dropPanel-content rey-elWishlist-content">';
			...
			$title_tag = apply_filters('reycore/elementor/header-wishlist/title_tag', 'h4');
	...
	protected function render() {
		...
		reyCoreAssets()->add_styles(['rey-wc-header-account-panel-top', 'reycore-widget-header-wishlist-styles']);
