...
class BasicSlider extends \ReyCore\Elementor\WidgetsBase {
	...
	public function rey_get_script_depends() {
		return [ 'rey-splide', 'reycore-widget-basic-slider-scripts' ];
	}
	...
	protected function controls__slides(){
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					// 'acf_gallery' => esc_html__( 'ACF Gallery (Custom field)', 'rey-core' ),
				],
			]
		);
		...
		$this->add_control(
			'acf_gallery_images',
			[
				'label' => esc_html__( 'Select ACF Gallery Field', 'rey-core' ),
		...
		$repeater->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} {{CURRENT_ITEM}}' => '--bg-image: url({{URL}})',
				],
			]
		);
	...
	public function __label(){
		...
		if( ! ($label = $this->_slide['label']) ){
			return;
		}

		printf('<%2$s class="__captionEl __captionLabel">%1$s</%2$s>', $label, self::$defaults['label_tag']);
	...
	public function render_item(){
		...
		if( isset($this->_slide['image']['id']) && $img_id = $this->_slide['image'] ){

			$attributes = [
				'class' => '__media'
			];

			if( isset($this->_slide['ken_burns']) && ($ken_burns = $this->_slide['ken_burns']) ){
				$attributes['data-kb'] = $ken_burns;
			}
	...
	protected function render() {
		...
		reyCoreAssets()->add_styles(['reycore-widget-basic-slider-styles', 'rey-splide']);
	...
	public function get_acf_gallery(){

		if( ! class_exists('\ReyCore\ACF\Helper') ){
			return;
		}

		if( ! isset($this->_settings['acf_gallery_images']) ){
			return;
		}

		if( ! ($key = $this->_settings['acf_gallery_images']) ){
			return;
		}

		$gallery = \ReyCore\ACF\Helper::get_field_from_elementor($key);
