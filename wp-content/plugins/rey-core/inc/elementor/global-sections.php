...
class GlobalSections
{
	...
	public static $collected_ids = null;
	public static $__collecting_ids = null;
	 ...
	 * @since 1.0.0
	 **/
	public static function do_section($post_id = null, $with_css = false, $disable_container_spacing = false)
	{
		if( is_null($post_id) ){
			return;
		}
		...
		if( class_exists('\Elementor\Plugin') && is_callable( '\Elementor\Plugin::instance' ) ) {
			...
			if( ! is_null(self::$collected_ids) ){
				self::$collected_ids[] = $post_id;
			}

			$GLOBALS['global_section_id'] = $post_id;

			$gs_markup = $edit_markup_start . $elementor_instance->frontend->get_builder_content_for_display( $post_id, $with_css ) . $edit_markup_end;
			...
			$SR = [
				'gs_type' => sprintf(' data-elementor-gstype="%s"', esc_attr($_gstype)),
				'el_selector' => sprintf(' data-page-el-selector="body.elementor-page-%d"', $post_id),
			];

			if( current_user_can('edit_posts') ){
				$gs_types = self::get_global_section_types();
				$gs_type = isset($gs_types[$_gstype]) ? $gs_types[$_gstype] : $_gstype;
				$SR['edit_mode'] = sprintf(' data-elementor-name="%s" data-elementor-gstype-title="%s"', esc_attr(get_the_title($post_id)), esc_attr($gs_type) );
			}

			if( $disable_container_spacing ){
				$SR['disable_container_spacing'] = ' data-disable-padding';
			}

			foreach ($SR as $replace) {
				$gs_markup__search = sprintf(' data-elementor-id="%d"', $post_id);
				$gs_markup = preg_replace('/' . $gs_markup__search . '/', $gs_markup__search . $replace, $gs_markup, 1);
			}
	...
	public static function collect_start(){

		if( self::$__collecting_ids ){
			return;
		}

		self::$__collecting_ids = true;
		self::$collected_ids = [];
	}

	public static function collect_end(){

		if( ! is_null(self::$collected_ids) ){
			return array_unique(self::$collected_ids);
		}

		self::$collected_ids = null;
		self::$__collecting_ids = null;
	...
	public static function get_posts( $args = [] ) {
		...
		if( false === ($items = get_transient(self::GS_TRANSIENT)) ){
	...
	public static function load_sections_css() {
		...
		if( !empty( $sections_css ) ) {
			...
			reyCoreAssets()->add_styles('reycore-elementor-frontend');
