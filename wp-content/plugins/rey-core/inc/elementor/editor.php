...
class Editor
{
	...
	public function add_elementor_widget_categories( $elements_manager ) {
		...
		$elements_manager->add_category(
			'rey-header',
			[
				'title' => sprintf(__( '%s - Header', 'rey-core' ), reycore__get_props('theme_title')),
				'icon' => 'fa fa-plug',
				// 'active' => false,
			]
		);
		...
		$elements_manager->add_category(
			'rey-theme',
			[
				'title' => reycore__get_props('theme_title'),
				'icon' => 'fa fa-plug',
			]
		);
		$elements_manager->add_category(
			'rey-theme-covers',
			[
				'title' => sprintf(__( '%s - Covers (sliders)', 'rey-core' ), reycore__get_props('theme_title')),
				'icon' => 'fa fa-plug',
			]
		);
		if( class_exists('\WooCommerce') ){
			$elements_manager->add_category(
				'rey-woocommerce',
				[
					'title' => sprintf(__( '%s - WooCommerce', 'rey-core' ), reycore__get_props('theme_title')),
					'icon' => 'fa fa-plug',
				]
			);
		}
	...
	public function editor_scripts() {
		...
		wp_localize_script('rey-core-elementor-editor', 'reyElementorEditorParams', [
			...
			'optimized_dom' => Helper::get_props('optimized_dom'),
	...
	public function add_grid_type_control($section){
		...
		$section->add_control( [
			...
			'label'       => esc_html__( 'Elementor Grid Type', 'rey-core' ),
			'help' => [
				__( 'Rey\'s grid override will make the column gaps actually act like gaps instead of column inner padding. This should reduce the need for customizing each column to adjust its padding and margins.', 'rey-core' )
			],
			'default'     => 'rey',
			'choices'     => [
		...
		if( \ReyCore\Elementor\Helper::get_props('container') ):
