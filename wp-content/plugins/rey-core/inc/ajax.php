...
class Ajax
{
	...
	public function define_ajax() {
		...
		if ( ! defined( 'REY_DOING_AJAX' ) ) {
			define( 'REY_DOING_AJAX', true );
		}
	...
	public function handle_ajax_request() {
		...
		if( isset($deprecated_hooks[$action_id]) ){
			$maybe_check_nonce = apply_filters($deprecated_hooks[$action_id], $maybe_check_nonce);
		}
	...
	private function get_transient_data($transient, $action_id, $action_data){
		...
		// unique hash
		$transient_unique_id = \ReyCore\Helper::hash($action_data);
	...
	public function flush_ajax_transients(){
		\ReyCore\Helper::clean_db_transient( self::AJAX_TRANSIENT_NAME );
	}
	...
	public function adminbar_add_node( $nodes ){
		...
		if( ! \ReyCore\Helper::check_db_transients( self::AJAX_TRANSIENT_NAME ) ){
			return $nodes;
		}
