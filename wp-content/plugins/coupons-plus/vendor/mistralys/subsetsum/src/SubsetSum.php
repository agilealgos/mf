...
class SubsetSum
{
    ...
    */
    private function searchRecursive(array $numbers, array $currentStack=array())
    {
        $s = array_sum($currentStack);
        $search = $this->getSum();

        (boolean) $thisMatchAlreadyExists = in_array($currentStack, $this->matches);
        ...
        {
            asort($currentStack); // ensure the numbers are always sorted
            if ($this->isindexed($currentStack)) {
        ...
        foreach ($numbers as $key => $number) {
            ...
            // cant use array shift because it messes up with indexed (numeric) arrays            
