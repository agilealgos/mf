<?php
	...
	function wc_os_after_group_category_headings_callback(){
			...
			global $wpdb;
				...
				$results_query = "SELECT p.ID,pm.meta_key FROM `" . $wpdb->posts . "` p LEFT JOIN `" . $wpdb->postmeta . "` pm ON p.ID=pm.post_id WHERE p.post_type = 'shop_order' AND pm.meta_key!='NULL' GROUP BY pm.meta_key ";				
	...
	function filter_posts_per_meta_value( $query ) {
		...
		if( $pagenow == 'edit.php' && $post_type == 'shop_order' && isset($_GET['parent_order']) && $_GET['parent_order'] != '' && is_numeric($_GET['parent_order']) ) {
			
			$parent_order = sanitize_wc_os_data($_GET['parent_order']);
    ...
    if(!function_exists('wc_os_after_order_split_callback')){
        ...
        function wc_os_after_order_split_callback($new_order_ids=array(), $originalorderid=0){
			...
			$is_shop_order = (get_post_type($originalorderid)=='shop_order');
            ...
            if(!empty($new_order_ids)){
                ...
                foreach ($new_order_ids as $new_oder){
					...
					$is_shop_order = (get_post_type($new_oder)=='shop_order');
					...
					if(class_exists('WCML_Emails')){
						global $sitepress;
						$wpml_language = get_post_meta($new_oder, 'wpml_language', true);
                    ...
                    if($order_split_created || $order_split_created_admin || $order_split_created_vendor){
						...
						delete_post_meta($new_oder, '_new_order_email_sent');
	...
	if(!function_exists('wc_os_add_funds_used_child')){
		...
		function wc_os_add_funds_used_child($original_id, $child_orders){
			...
			$total_funds = get_post_meta($original_id, '_funds_used', true);
			$funds_used = 0;
			...
			if(!empty($child_orders)){
				...
				foreach ($child_orders as $child_order_id){
					...
					update_post_meta($child_order_id, '_funds_used', $funds_used_by_current);
					update_post_meta($child_order_id, '_funds_removed', 1);
    ...
    function wc_os_update_lookup_tables($order_id, $table_name_without_prefix){
		...
		if($parent_order_id){
			...
			if(empty($parent_order_items)){
				...
				update_post_meta($order_id, '_wc_os_report_included', 'yes');
				update_post_meta($parent_order_id, '_wc_os_report_included', 'no');
		...
		$wc_os_child_order =  get_post_meta($order_id, '_wc_os_child_order', true);
		$force_sync =  get_post_meta($order_id, '_wc_os_force_sync', true);
    ...
    function wc_os_add_order_meta_after_split($new_order_ids, $original_order){
		...
		$_wc_os_parent_order = get_post_meta($original_order, '_wc_os_parent_order', true);

        if($original_order && !$_wc_os_parent_order){

			update_post_meta($original_order, '_wc_os_parent_order', 'yes');
        ...
        if(!empty($new_order_ids)){
            ...
            foreach ($new_order_ids as $order_id){
                ...
                update_post_meta($order_id, '_wc_os_child_order', 'yes');
                update_post_meta($order_id, '_wc_os_report_included', 'no');
				...
				if ($wc_os_ie_selected=='group_by_vendors'){
					if(in_array('wcfm_marketplace_orders', $wpdb->tables)) {
	...
	function wc_os_update_child_orders_stats($order_id){
		...
		$wc_os_parent_order = get_post_meta($order_id, '_wc_os_parent_order', true);
		if($wc_os_parent_order){
				...
				if(!empty($get_child_ids)){
					foreach($get_child_ids as $child_id){
						...
						update_post_meta($child_id, '_wc_os_force_sync', true);
						...
						update_post_meta($child_id, '_wc_os_report_included', 'yes');
						delete_post_meta($child_id, '_wc_os_force_sync');
		...
		function wc_os_method_based_default_order_status($default_action='', $order_id=0, $expected=''){	
			...
			switch($wc_os_settings['wc_os_ie']){
				...
				case 'io':
					...
					$default_action = (array_key_exists('default_stock_status', $io_options) ? $io_options['default_stock_status'] : $default_action);					

					if(!empty($order_meta) || $order_id==0){
						if(array_key_exists('_wos_in_stock', $order_meta) || $expected=='_wos_in_stock'){
							$default_action = (array_key_exists('in_stock_status', $io_options) ? $io_options['in_stock_status'] : $default_action);
						}
						if(array_key_exists('_wos_out_of_stock', $order_meta) || $expected=='_wos_out_of_stock'){
							$default_action = (array_key_exists('out_stock_status', $io_options) ? $io_options['out_stock_status'] : $default_action);
						}
					}
...
<?php			
    ...
    if(!function_exists('wc_os_get_statuses_select_html')){
        function wc_os_get_statuses_select_html($input_name, $field_prefix, $wc_os_order_statuses, $package_title=true, $label_prefix='', $type=''){
            $wos_packages_strings = wc_os_get_packages_strings();
...
<?php endif; ?>
        ...
        function wc_os_update_group_order_status($child_orders=array(), $original_order_id=0){
            ...
            if(!empty($child_orders)){
                ...
                foreach($child_orders as $child_id){
					$_wos_custom_status_update = get_post_meta($child_id, '_wos_custom_status_update', true);
                    $group_name = get_post_meta($child_id, '_wos_group_name', true);
                    ...
                    if($group_name && $_wos_custom_status_update!=$group_status){
                        ...
                        if($group_status && function_exists('wc_os_update_order_status')){
                            ...
                            update_post_meta($child_id, '_wos_custom_status_update', $group_status);
	...
	if(!function_exists('wc_os_add_prefix')){
		function wc_os_add_prefix($str, $prefix=''){
