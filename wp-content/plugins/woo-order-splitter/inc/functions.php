<?php if ( ! defined( 'ABSPATH' ) ) exit;
    ...
    class wc_os_order_splitter {
		...
		function __construct() {
			...
			global $wc_os_bulk_instantiated, $wc_os_pro;
			...
			if(!$wc_os_bulk_instantiated){
				...
				add_filter( 'manage_shop_order_posts_custom_column', array($this, 'wc_os_orders_list_columns_content') );
		...
		public function wc_os_orders_list_columns_content( $column ) {
			global $post;
			...
			switch($column){
				case 'split_from':
					...
					if($wc_os_order_splitf_column){
						$splitted_from = get_post_meta($post->ID, 'splitted_from', true);
						if($splitted_from)
						echo '<a href="'.get_edit_post_link( $splitted_from ).'">#'.$splitted_from.'</a>';
						else
				...
				case 'clone_from':	
					...
					$wc_os_order_clonef_column = array_key_exists('wc_os_order_clonef_column', get_option('wc_os_general_settings', array()));
					if($wc_os_order_clonef_column){
						$cloned_from = get_post_meta($post->ID, 'cloned_from', true);
						...
						if($cloned_from){
							...
							echo '<a href="'.get_edit_post_link( $cloned_from ).'">#'.$cloned_from.'</a>';
						
						}else{
						
							$is_combined = get_post_meta($post->ID, 'auto_combined', true);
							$wc_os_child_order = get_post_meta($post->ID, '_wc_os_child_order', true);
				...
				case 'order_number_s':
					...
					if(get_splitted_order_title_status() && function_exists('wc_os_get_order_title_html')){
						...
						wc_os_get_order_title_html($post->ID);
				...
				case 'items_count':
					...
					$_wc_os_total_items =  get_post_meta($post->ID, '_wc_os_total_items', true);
						...
						switch($wc_os_settings['wc_os_ie']){
							default:								
								...
								if($order_id){
									...
									$get_post_meta = get_post_meta($originalorderid);
									if(!array_key_exists('split_status', $get_post_meta)){
		...
		public function clone_order($originalorderid = null, $force=false){
			...
			$_wos_cloned = get_post_meta($originalorderid, '_wos_cloned', true);
			...
			if ( is_wp_error( $order_id ) ) {
				...
				if($force){	
					update_post_meta($originalorderid, '_wos_cloned', true);
				}
			}
		...
		*/
		...
		public function split_order_logic($originalorderid = null, $wc_os_cart = false, $return_ids = false){
			...
			$get_post_meta = ($originalorderid?get_post_meta($originalorderid):array());
			$_order_key = ($originalorderid?get_post_meta($originalorderid, '_order_key', true):false);
			...
			delete_post_meta($originalorderid, '_wc_os_total_items');
			...
			if(!empty($wc_order_items)){
				...
				$wos_forced_ie = ($originalorderid?get_post_meta($originalorderid, 'wos_forced_ie', true):false);	
...
//									$this->wos_remove_order_item($originalorderid, $this_include_items);
						...
						case 'io':
							...
							if($wc_os_pro && class_exists('wc_os_bulk_order_splitter')){
								...
								$items_io = $classObj->separate_io_items($wc_order_items, $this->include_items_qty, $wc_os_cart, $this->re_split);
								...
								if( $wc_os_cart || ($in_stock_exists && $backorder_exists) ){	
									...
									delete_post_meta($originalorderid, '_wos_out_of_stock');
									update_post_meta($originalorderid, '_wos_in_stock', true);
									$proceed_split = true;
									...
									if($backorder_exists){
										delete_post_meta($originalorderid, '_wos_in_stock');
								...
								if((isset($items_io['in_stock']) && !empty($items_io['in_stock']))){ //create order of in-stock items
									...
									if($wc_os_cart){
										...
										if($wc_os_effect_parent){
											...
											if(is_admin()){
												update_post_meta($originalorderid, '_wos_update_order_item', array('include_items'=>$this->include_items, 'include_items_qty'=>$this->include_items_qty));
												...
												if($items_io['backorder_split_required'] && empty($backorder_only)){
													...
													//if($split_status_value){										
														...
														$removal_arr[$order_id] = $split_status_value;
                                            ...
                                            if(isset($items_io['in_stock']) && !empty($items_io['in_stock']) && !empty($items_io['in_stock']['items'])){
                                                ...
                                                if ( is_wp_error( $order_id ) ) {
                                                    ...
                                                    update_post_meta($order_id, '_wos_in_stock', true);
								...
								if(isset($items_io['backorder']) && !empty($items_io['backorder']) && is_array($items_io['backorder']['items']) && !empty($items_io['backorder']['items'])) //create order of backorder items
								{
									...
									if($items_io['backorder_split_required']){
										...
										if($wc_os_cart){
											...
											//pree($backorder_items_arr);	
											//pree($backorder_items_qty_arr);	
											
											foreach($backorder_items_arr as $backorder_item){
												$this->include_items = array();
												$this->include_items_qty = array();
											
												$this->include_items = (is_array($backorder_item)?$backorder_item:array($backorder_item));
												
												if(is_array($backorder_item)){
													$backorder_item_parked = current($backorder_item);
													$backorder_items_qty_arr_parked = current($backorder_items_qty_arr);
													
													if(array_key_exists($backorder_item_parked, $backorder_items_qty_arr_parked)){
														$backorder_item = $backorder_item_parked;
														$backorder_items_qty_arr = $backorder_items_qty_arr_parked;
													}
												...
												//wc_os_logger('debug', $backorder_item, true);
												//wc_os_logger('debug', $backorder_items_qty_arr, true);
												
												
												$this->include_items_qty = array($backorder_item=>(array_key_exists($backorder_item, $backorder_items_qty_arr)?$backorder_items_qty_arr[$backorder_item]:1));
												
												//wc_os_logger('debug', $this->include_items_qty, true);
												
												//pree($this->include_items);		
												//pree($this->include_items_qty);										
												//continue;
												...
												$order_split_status = wc_os_add_prefix($order_split_status, 'wc-');
	
												$order_data =  array(
													'post_type'     => 'shop_order',
													'post_status'   => $order_split_status,
													'ping_status'   => 'closed',
													'post_author'   => $user_id,
													'post_password' => uniqid( 'order_' ),
												);
												...
												if ( is_wp_error( $order_id ) ) {
													...
													delete_post_meta($order_id, '_wos_in_stock');
	...
	//}
											...
											if($order_id_new){
												...
												delete_post_meta($order_id_new, 'wc_os_order_splitter_cron');//quantity_split //in case this meta_key was inherited from parent order
								...
								if($wos_forced_ie && $wos_delete_order){
									...
									if(!empty($category_arr)){
										foreach($category_arr as $cat_group=>$items_arr){
											...
											$order_id = wp_insert_post( apply_filters( 'woocommerce_new_order_data', $order_data_args), true );
				...
				if($originalorderid){
					...
					if(!empty($new_order_ids)){
						...
						foreach($new_order_ids as $order_id){
							...
							update_post_meta($order_id, '_wc_os_total_items', (is_object($new_order_obj)?count($new_order_obj->get_items()):0));
							...
							update_post_meta($order_id, '_wos_calculate_totals', true);		
						}	
						...
						delete_post_meta($originalorderid, 'wc_os_order_splitter_cron');//split_order_logic
		...
		*/			
			...
			if(!empty($to_delete)){
				if($delete){
					foreach($to_delete as $item_id){
						...
						update_post_meta($originalorderid, '_wc_os_effected_order', true);				
					}	
					$original_order = wc_get_order($originalorderid);
				...
				if(!empty($wc_os_products_arr)){
					...
					if(!empty($orders_arr)){
						...
						foreach($orders_arr as $orders_items){
								...
								$order_id_new = wp_insert_post( apply_filters( 'woocommerce_new_order_data', $order_data_args), true );
								...
								if ( is_wp_error( $order_id_new ) ) {									
									...
									update_post_meta($order_id_new, 'splitted_from', $originalorderid);
								}																				
		...
		public function split_order($originalorderid = null, $wc_os_products=array()){
			...
			if(!wc_os_order_split($originalorderid))
			return;
			...
			if($originalorderid>0){
				...
				$_order_stock_reduced = get_post_meta($originalorderid, '_order_stock_reduced', true);
					...
					if(!$group_selected_items && !empty($orders_arr)){
						...
						foreach($orders_arr as $orders_items){
								...
								if ( is_wp_error( $order_id_new ) ) {									
									...
									update_post_meta($order_id_new, '_order_stock_reduced', $_order_stock_reduced);
									update_post_meta($order_id_new, 'wc_pos_order_type', sanitize_wc_os_data($wc_pos_order_type)); // NO QTY. SPLIT	
									...
									wc_os_update_split_status($order_id_new, 2909);
									update_post_meta($order_id_new, 'splitted_from', $originalorderid);
									update_post_meta($order_id_new, '_wc_os_total_items', count($this->include_items));
				...
				delete_post_meta($originalorderid, 'wc_os_order_splitter_cron');//split_order
		...
		public function cloned_order_data($order_id, $originalorderids = null, $clone_order=true, $reduce_stock=false, $clone_shipping=true, $parent_iter='', $update_order_status=false){
			...
			foreach( $originalorderids as $originalorderid ) {	$iter++;
				...
				delete_post_meta($this->original_order_id, '_wc_os_total_items');
				...
				if(!$wc_os_debug){
					...
					update_post_meta($order_id, '_wc_os_total_items', count($order->get_items()));
				...
				if($clone_order){
					...
					if(method_exists($classObj, 'add_order_items')){
						...
						$classObj->add_order_items($order);
		...
		public function meta_keys_clone_from_to($order_id_to=0, $order_id_from=0){
			...
			if($order_id_from && $order_id_to){
				...
				$this->wos_update_post_meta( $order_id_to, '_billing_address_index', get_post_meta($order_id_from, '_billing_address_index', true) );
				$this->wos_update_post_meta( $order_id_to, '_shipping_address_index', get_post_meta($order_id_from, '_shipping_address_index', true) );
				...
				$exclude_arr = array('vxc_zoho_order');
									...
									case '_barcode_image':
										if(class_exists('WooCommerce_Order_Barcodes') && function_exists('wos_barcode')){
											...
											if( 'yes' == $WooCommerce_Order_Barcodes->barcode_enable ) {
												...
												$_barcode_text = update_post_meta($order_id_to, '_barcode_text', $barcode_string);
				...
				if(!empty($new_order->get_items())){
						...
						if(in_array($this->auto_split, $wc_os_meta_handling_arr)){
							...
							if($is_wc_booking && !empty($this->booking_ids)){
							    ...
							    if($current_booking_id){
							        update_post_meta($current_booking_id, '_bkap_order_item_id', $item_id);
                                }
                            }							
					...
					update_post_meta($order_id_to, '_old_order_items_mapped', $old_order_items_mapped);
                }
								    ...
								    foreach ($itemmeta_row_arr['items'] as $product_item => $flag){
								        if($flag && in_array($product_item, $order_items)){
                                            ...
                                            if ( $item_id ) {
												...
												update_post_meta($order_id_to, '_wos_keep_shipping', $item_id);
                                            }
...
// Returns success message on clone completion
        ...
        public function clone_order_coupons($order, $original_order){
            ...
            if(is_object($original_order) && method_exists($original_order, 'get_coupon_codes')) {
				...
				$wc_os_skip_default_coupon = get_post_meta($original_order->get_id(), '_wc_os_skip_default_coupon', true);
		...
		public function clone_order_items($order, $original_order, $clone_order=false){
			...
			global $wc_os_pro, $yith_pre_order, $wc_os_debug, $wc_os_tax_cost, $wc_os_meta_handling_arr, $wpdb;
			...
			foreach($original_order->get_items() as $order_key => $values){
				...
				$unit_price = ($unit_price!=$product->get_price()?$unit_price:$product->get_price());
		...
		public function clone_order_items_by_item($order, $original_order){
            ...
            global $wc_os_pro, $yith_pre_order, $wc_os_debug, $wc_os_tax_cost, $wc_os_meta_handling_arr;
	    ...
	    public function splitted_order_items($order, $original_order, $product_id, $variation_id, $qty=false, $total=false, $order_item_id=0){
			...
			global $wc_os_tax_cost, $wc_os_general_settings;
		...
		public function clone_order_shipping_items($order_id=0, $original_order=array(), $qty=false, $clone_shipping=false){
			...
			if(empty($original_order) && !$order_id){ //TROUBLESHOOTING SECTION
				...
				$parent_order_id = get_post_meta($order_id, 'splitted_from', true);
			...
			$_vendor_id = get_post_meta($order_id, '_vendor_id', true);
	...
	function wc_os_is_orders_list(){
		...
		return (is_admin() && $pagenow=='edit.php' && $post_type=='shop_order')?true:false;
...
add_action("woocommerce_checkout_order_processed", "wc_os_checkout_order_processed", 2, 1);
	...
	if(!function_exists('wc_os_crons_wos_calculate_totals')){
		function wc_os_crons_wos_calculate_totals($order_id=0){
			...
			global $wpdb;
			$cron_query = "SELECT p.ID FROM $wpdb->postmeta mt RIGHT JOIN $wpdb->posts p ON p.ID=mt.post_id AND p.post_type='shop_order' WHERE p.post_status NOT IN ('trash') AND (mt.meta_key='_wos_calculate_totals')";
			
			if($order_id){ $cron_query = "SELECT p.ID FROM $wpdb->posts p WHERE p.post_status NOT IN ('trash') AND p.ID IN (".esc_sql($order_id).") AND p.post_type='shop_order'"; }
			...
			if(!empty($wc_os_order_key_cron)){
				foreach($wc_os_order_key_cron as $all_crons_items){			
					...
					if(is_object($order_data) && !empty($order_data)){
						...
						delete_post_meta($all_crons_items->ID, '_wos_calculate_totals');
						update_post_meta($all_crons_items->ID, '__wos_calculate_totals_for_tax', true);
	...
	function __wos_change_order_received_text($order_id=0, $parent_order=array(), $deleted=true){
		...
		if(!empty($posts_array)){
			...
			foreach($posts_array as $post_data){
				...
				$_wos_keep_shipping = get_post_meta($post_data->ID, '_wos_keep_shipping', true);
				$_wos_remove_shipping = get_post_meta($post_data->ID, '_wos_remove_shipping', true);
				...
				if($wc_os_shipping_cost){
					...
					$_wc_os_set_status = get_post_meta($child_order->get_id(), '_wc_os_set_status', true);
					$_order_shipping = get_post_meta($child_order->get_id(), '_order_shipping', true);
					...
					if($_wc_os_set_status){
						...
						delete_post_meta($child_order->get_id(), '_wc_os_set_status');
					}
				}
...
?>
	...
	<h2 class="woocommerce-order-details__title"><?php echo apply_filters('wc_os_child_order_heading', isset($wos_cart_notices['co_heading'])?$wos_cart_notices['co_heading']:__('Child', 'woo-order-splitter').' '.(count($posts_array)>1?__('Orders', 'woocommerce'):__('Order', 'woocommerce'))); ?></h2>
	...
	foreach($posts_array as $post_data){ $child_order = wc_get_order($post_data->ID); 
		...
		$_order_shipping_tax = (get_post_meta($post_data->ID, '_order_shipping_tax', true));
		$_order_shipping_tax = ($_order_shipping_tax?$_order_shipping_tax:0);

		$deposit_value = (get_post_meta($post_data->ID, 'deposit_value', true));
		$deposit_value = ($deposit_value?$deposit_value:0);
		
		$due_payment = (get_post_meta($post_data->ID, 'due_payment', true));
	...
	function wc_os_checkout_order_processed($order_id){
		...
		if(
			...
			(
				...
				||
					((isset($_GET['post_type']) && $_GET['post_type']=='shop_order' && (isset($_GET['action']) && $_GET['action']=='split')))
			)
		){
			return;
		}
		...
		if($wc_os_shipping_methods && wc_os_get_session( 'wc_os_customer_permitted')=='on'){
			update_post_meta($order_id, '_wc_os_shipping_cost', wc_os_get_session('wc_os_calculated_shipping_cost'));
			update_post_meta($order_id, '_wc_os_parcels_array', wc_os_get_session('wc_os_parcels_array'));
	...
	function wc_os_get_customer_permission_status($order_id=0){		
		...
		if($wc_os_customer_permission && $order_id){				
			...
			$wc_split_permit = get_post_meta($order_id, '_wc_os_customer_permitted', true);

		}
		$wc_split_permit = ($wc_split_permit=='yes' || $wc_split_permit==true);
	...
	function wos_get_valid_status($post, $order_id = 0){
		...
		if(is_object($post) && isset($post->post_type) && !in_array($post->post_type, array('shop_order'))){
			...
			if(!$valid_post_type && $post->post_parent>0){
				...
				if(is_object($expected_order) && isset($expected_order->post_type) && in_array($expected_order->post_type, array('shop_order'))){
					$valid_post_type = true;
	...
	function order_details_page_saved( $order_id, $post=array() ) {
		...
		if(isset($_POST['_wc_os_status_locked']) && $order_id){
			...
			update_post_meta($order_id, '_wc_os_status_locked', $_wc_os_status_locked);
		...
		if(isset($_POST['wc_os_customer_permitted'])){
			...
			update_post_meta($order_id, '_wc_os_customer_permitted',  $wc_os_customer_permitted);
		...
		if(wc_os_order_split($order_id) && $split_permission_status){
			...
			if(
				...
				switch($wc_os_settings['wc_os_ie']){
						...
						case 'default':
							...
							if($order_details_page && wc_os_order_split($order_id))
			...
			}elseif((!is_admin() && array_key_exists('wc_os_auto_forced', get_option('wc_os_general_settings', array())))){
			...
			$_wc_os_shipping_method_old = get_post_meta($order_id, '_wc_os_shipping_method',  true);
			
			if(!is_admin() && !$_wc_os_shipping_method_old){
				update_post_meta($order_id, '_wc_os_shipping_method',  $_wc_os_shipping_method);
			}
	...
	function wc_os_get_order_vendor($order_id=0){
        ...
        $_vendor_id = get_post_meta($order_id, '_vendor_id', true);
        $_vendor_by_term = get_post_meta($order_id, '_vendor_term', true);
	...
	function wc_os_process_parent_email($order_id){
		...
		if(!empty($post_meta)){
			...
			if($is_parent_order){
					...
					$_wos_parent_email = get_post_meta($order_id, '_wos_parent_email', true);
					...
					if(!$has_child_orders && !empty($_wos_parent_email)){
						...
						$_customer_registered_id = get_post_meta($order_id, '_customer_user', true);
						...
						delete_post_meta($order_id, '_wos_parent_email');
	...
	function wc_os_front_scripts() {	
		...
		global $wp, $wc_os_settings, $wc_os_general_settings, $woocommerce, $wc_os_minified_js, $wc_os_minified_css;
	
		$wc_os_shipping_methods = array_key_exists('wc_os_shipping_methods', $wc_os_general_settings);
		$wc_os_customer_permission = array_key_exists('wc_os_customer_permission', $wc_os_general_settings);

		$front_script = ($wc_os_minified_js ? 'js/front-scripts-min.js' : 'js/front-scripts.js');
		$front_style = ($wc_os_minified_css ? 'css/front-style-min.css' : 'css/front-style.css'.'?t='.time());
		...
		$translation_array = array(
			...
			'total_cart_items' => count(( ! WC()->cart->is_empty() )?WC()->cart->get_cart():array())
	...
	function wc_os_admin_scripts() {
		...
		global $wc_os_css_arr, $wc_os_settings, $post, $wc_os_cust, $wos_actions_arr, $wc_os_general_settings, $wp_scripts, $wc_os_minified_js, $wc_os_minified_css, $post_type, $wc_os_order_statuses_class, $wc_os_pro, $wuos_wos, $pagenow;
		...
		$subscription_split = in_array('subscription_split', $wc_os_settings['wc_os_additional']);
		$is_edit_order_page = ($pagenow=='post.php' && is_object($post) && ($post->post_type=='shop_order' || ($subscription_split && $post->post_type=='shop_subscription')));
		...
		$is_edit_order_page = (is_object($post) && isset($post->post_type) && $post->post_type=='shop_order' && isset($_GET['action']) && $_GET['action']=='edit');
		...
		if($post_type == 'shop_order'){
			...
			if($order_id){
				...
				$_wos_child_orders = get_post_meta($order_id, '_wos_child_orders', true);
	...
	function wp_os_filter_bulk_actions_edit_shop_order( $array ) { 
...
<?php		
	...
	function wc_os_order_split_status_action($order=array(), $include_items=array()){
		...
		if(is_object($order)){
			$split_status_lock = apply_filters('wc_os_split_order_status_logic_hook', $order, $split_status_lock, $include_items);
		}
	...
	function wos_shop_page_pre_get_posts( $query ) {
		...
		if ($query->is_main_query() && isset($_GET['vendor']) && is_numeric($_GET['vendor']) && $_GET['vendor']>0) :
			$query->set( 'author', sanitize_wc_os_data($_GET['vendor']) );
		endif;
		
		global $current_screen, $wc_os_order_statuses_class, $wpdb;
		
		if(is_admin()){
			
			$filter_order_items = (is_object($current_screen) && $current_screen->post_type=='shop_order' && !array_key_exists('post_status', $_GET));
			...
			if($filter_order_items){
				...
				if($query->is_main_query() && !empty($order_statuses_available_only)){	
					
					$query->set('post_status', $order_statuses_available_only);
				}
	...
	function wc_os_links($actions, $post=array()){
		...
		if (is_object($post) && $post->post_type=='shop_order' && wc_os_order_cloning()) {
		...
		if (is_object($post) && $post->post_type=='shop_order' && wc_os_order_split($post->ID)) {
	...
	function sv_wc_add_order_meta_box_action( $actions ) {
		...
		$get_post_meta = get_post_meta($theorder->get_order_number());
	...
	function add_wc_os_order_status_actions_button( $actions, $order ) {
		...
		$conflict_status = get_post_meta($order->get_order_number(), 'conflict_status', true);
		...
		if ( $status_lock_released && wc_os_order_split($order->get_id()) ) {
			...
			$get_post_meta = get_post_meta($order->get_id());
			...
			switch($wc_os_settings['wc_os_ie']){
				...
				case 'group_cats':
					...
					if($wc_os_woocommerce_shipping_multiple_addresses){
						$_multiple_shipping = get_post_meta($order->get_id(), '_multiple_shipping', true);
	...
	function wos_forced_ie(){
		if(isset($_POST['order_action'])){
			...
			$order_data = wc_get_order( $order_id );
			if(!empty($order_data)){
				update_post_meta($order_id, 'wos_forced_ie', sanitize_wc_os_data($_POST['order_action']));
	...
	if(!function_exists('wos_quick_split')){
		function wos_quick_split(){
			...
			if($wc_os_shipping_cost){
				...
				if(!empty($child_orders_by_order_id)){
					foreach($child_orders_by_order_id as $child_order_id){
						$_wc_os_set_status = get_post_meta($child_order_id, '_wc_os_set_status', true);
						...
						if($_wc_os_set_status){
							...
							delete_post_meta($child_order_id, '_wc_os_set_status');
						}
					}
				}
			...
			$crons_order_id = (wc_os_crons($query_string['order_id'])==$query_string['order_id']);
			$ret['query_string_order_id'] = $query_string['order_id'];
			$ret['order_id'] = ($crons_order_id?$query_string['order_id']:0);
			$ret['refresh'] = ($ret['order_id']>0?true:false);
    ...
    if(!function_exists('wc_os_update_method_order_status')){
        function wc_os_update_method_order_status($child_order, $original_order_id){
            ...
            if($parent_status){
				...
				update_post_meta($original_order_id, '_wos_custom_status_update', $parent_status);
            }
            ...
            if($child_status){
                ...
                if(!empty($child_order)){
                    foreach($child_order as $child_order_id)
						...
						update_post_meta($child_order_id, '_wos_custom_status_update', $child_status);
					...
					update_post_meta($child_order, '_wos_custom_status_update', $child_status);
		...
		function wc_os_set_empty_order_status($order_id){
			...
			if($order_id){
				...
				if(empty($order_items)){	
					...
					update_post_meta($order_id, '_not_empty', true);
				}
	...
	if(!function_exists('wc_os_apply_coupon_child_orders')){
        ...
        function wc_os_apply_coupon_child_orders($child_ids = array(), $original_order_id = 0){
            ...
            $actual_effect_parent = get_post_meta($original_order_id, '_wc_os_effected_order', true);
...
?>
	...
	if(!function_exists('wc_os_status_change_cron')){
		...
		function wc_os_status_change_cron($new_status, $old_status, $post){
			...
			if(is_object($post) && isset($post->post_type) && $post->post_type == 'shop_order'){
...
<?php
		if($pagenow=='post.php' && is_object($post) && $post->post_type=='shop_order'){
			...
			$_wc_os_status_locked = get_post_meta($post->ID, '_wc_os_status_locked', true);
...
<?php		
	...
	function wc_os_update_order_item_admin($order_id=0){
		if($order_id){
			...
			$_wos_update_rounds = get_post_meta($order_id, '_wos_update_rounds', true);
			$_wos_update_rounds = ($_wos_update_rounds?$_wos_update_rounds:0);
			$_wos_update_rounds++;
			$_wos_update_order_item = get_post_meta($order_id, '_wos_update_order_item', true);
			...
			if($_wos_update_order_item && is_array($_wos_update_order_item) && !empty($_wos_update_order_item)){
				update_post_meta($order_id, '_wos_update_rounds', $_wos_update_rounds);
				...
				if($_wos_update_rounds>1){
					delete_post_meta($order_id, '_wos_update_order_item');
					delete_post_meta($order_id, '_wos_update_rounds');
				}
				$order_data = wc_get_order( $order_id );
				$order_data->calculate_totals();
				$order_data->save();
	...
	if(!function_exists('wc_os_update_order_status')){
        function wc_os_update_order_status($order_id, $status=''){
            ...
            if(is_object($order_post) && $order_post->post_type = 'shop_order'){
	...
	function wc_os_update_split_status($order_id, $line_no='', $status=true, $splitted_orders=array()){
		...
		if(!in_array($split_status, array($status, '-'))){
			...
			if(in_array($status, array(true, false))){
				...
				update_post_meta($order_id, 'split_status', $status);
			}
	...
	if(!function_exists('wc_os_set_order_status')){
		function wc_os_set_order_status($order_id=0, $status_org='', $force=false, $line_no=0, $consider_child=false){
			...
			if($consider_child){
				update_post_meta($order_id, '_wc_os_parent_order', 'no');				
			}
			...
			//wc_os_logger('debug', $order_id.' - '.$function.' - ln. '.$line_no);
			$updated_status = '';
			
			if($order_id>0){
					
					$_wc_os_created_via = get_post_meta($order_id, '_created_via', true);
					$_wc_os_status_locked = get_post_meta($order_id, '_wc_os_status_locked', true);
					
					$do_not_change = ($_wc_os_created_via=='subscription' || $_wc_os_status_locked);
					if($do_not_change){
						delete_post_meta($order_id, '_wc_os_set_status');
					...
					if(!$status_org && !$force){
						...
						$is_parent_order = (get_post_meta($order_id, '_wc_os_parent_order', true)=='yes');
						...
						if($is_parent_order){
							...
							if($split_removal_case){
								$status_org = wc_os_order_split_removal_action('string', 9853);
					...
					if($status_org){
						...
						$_wc_os_set_status = get_post_meta($order_id, '_wc_os_set_status', true);
						...
						if(!$_wc_os_set_status || $force){
							...
							$status = apply_filters('wc_os_split_order_status_logic_hook', $order, $status);
							...
							if(is_object($order)){
								...
								if($wc_os_shipping_cost && !$wc_os_is_combine && !$force){
									...
									update_post_meta($order_id, '_wc_os_set_status', $status);
	...
	if(!function_exists('wc_os_set_splitter_cron')){
		function wc_os_set_splitter_cron($order_id=0, $status=true, $line_no=0){
			...
			if($order_id>0){
					$wc_os_order_splitter_cron = get_post_meta($order_id, 'wc_os_order_splitter_cron', true);	
					
					if($wc_os_order_splitter_cron!=$status){
	
						update_post_meta($order_id, 'wc_os_order_splitter_cron', $status);
	...
	function wc_os_reduce_order_stock( $order=array(), $stock_forced=false ) { // you get an object $order as an argument
		...
		if(is_object($order)){
			...
			$proceed = (in_array($wc_os_settings['wc_os_ie'], array('io', 'inclusive', 'default')) || $wc_os_reduce_stock);
			...
			if($proceed){
					...
					$_order_stock_reduced = get_post_meta($order_id, '_order_stock_reduced', true);
					
					if($_order_stock_reduced){
						return;
					}
					
					$_wc_os_child_order = (get_post_meta($order_id, '_wc_os_child_order', true)=='yes');
					
					$proceed_further = (!$_wc_os_child_order || $wc_os_reduce_stock);
					...
					if($proceed_further){
						...
						foreach( $items as $item_key=>$item ) {
							...
							if($items_ids[$item_key]['qty_stock_status']){
								...
								update_post_meta($item_id, '_stock', $updated_stock);
								update_post_meta($item_id, '_wc_os_stock_quantity', 0);
								
							
								
							}
							
							if($updated_stock>0 && $updated_stock<$child_order_note){
	...
	if(!function_exists('wos_admin_init_basic')){
		function wos_admin_init_basic(){
			...
			if(isset($_GET['get_keys'])){
				...
				wc_os_pre(get_post_meta($order_id));
	...
	add_action('admin_init', 'wc_os_admin_functions');
