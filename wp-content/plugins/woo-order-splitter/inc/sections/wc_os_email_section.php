...
<div class="nav-tab-content hides wc_os_emails_section tab-emails">
    ...
    <div class="wc_os_tab">
        ...
        <button class="wc_os_tab_links wc_os_active" data-target="wc_os_email_on_off"><?php _e('ON/OFF', 'woo-order-splitter'); ?></button>
        <button class="wc_os_tab_links" data-target="wc_os_email_placeholders"><?php _e('Placeholders', 'woo-order-splitter'); ?></button>
        <button class="wc_os_tab_links" data-target="wc_os_email_smtp_settings"><?php _e('SMTP Settings', 'woo-order-splitter'); ?></button>
        <button class="wc_os_tab_links" data-target="wc_os_email_test"><?php _e('Test Email', 'woo-order-splitter'); ?></button>
        <button class="wc_os_tab_links" data-target="wc_os_email_templates"><?php _e('Default Emails', 'woo-order-splitter'); ?></button>
...
?>		
	    ...
	    <?php if(function_exists('wos_email_notification')): ?>
            <ul>
                ...
                <li>
                    <input id="wc_os_parent_order_email" name="wc_os_email_settings[wc_os_parent_order_email]" type="checkbox" value="1" <?php echo($wc_os_parent_order_email_off?'checked="checked"':''); ?> /><label for="wc_os_parent_order_email"><?php _e("Do not send parent order email to anyone",'woo-order-splitter'); ?> <small>(<?php _e("If order splitted, normal order will get emails",'woo-order-splitter'); ?>)</small></label>
...
<?php	
		foreach($mails as $key=>$data){
			...
			$subject = (array_key_exists('subject', $data->form_fields)?$data->form_fields['subject']['placeholder']:'');
