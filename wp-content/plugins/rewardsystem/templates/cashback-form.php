...
<form id="encashing_form" class="encashing_form" method="post" enctype="multipart/form-data">
	<div class="rs_current_points_value">
		<p>
			<label><?php echo wp_kses_post( get_option( 'rs_total_points_for_cashback_request' ) ); ?></label>
		</p>
		<p>
			<input type="text" id="rs_available_points" name="rs_available_points" value="<?php echo wp_kses_post( $PointsToDisplay ); ?>" readonly="readonly">
		</p>
	</div>
	<div class="rs_encash_points_value">
		<p>
			<label><?php echo wp_kses_post( get_option( 'rs_encashing_points_label' ) ); ?></label>
		</p>
		<p>
			<input type="number" 
	...
	<div class="error" id="points_empty_error">
		<?php echo wp_kses_post( get_option( 'rs_error_message_points_empty_encash' ) ); ?>
	</div>
	<div class="error" id="points_number_error">
		<?php echo wp_kses_post( get_option( 'rs_error_message_points_number_val_encash' ) ); ?>
	</div>
	<div class="error" id ="points_greater_than_earnpoints_error">
		<?php echo wp_kses_post( get_option( 'rs_error_message_points_greater_than_earnpoints' ) ); ?>
	</div>
	<div class="error" id ="points_lesser_than_minpoints_error">
		<?php echo wp_kses_post( $ReplacedErrMsg ); ?>
	</div>
	<?php
	...
	if ( get_option( 'rs_cashback_' . $user_role . '_for_redeem_percentage', 100 ) ) {
		...
		<div class="rs_encash_currency_value">
			<p>
				<label><?php echo wp_kses_post( get_option( 'rs_encashing_currency_label', 'Currency Value' ) ); ?></label>
			</p>
			<p>                                    
	...
	<div class="rs_encash_points_reason">
		<p>
			<label><?php echo wp_kses_post( get_option( 'rs_encashing_reason_label' ) ); ?></label>
		</p>
		<p>
	...
	<div class="error" id ="reason_empty_error">
		<?php echo wp_kses_post( get_option( 'rs_error_message_reason_encash_empty' ) ); ?>
	</div>
	<?php if ( '3' == get_option( 'rs_select_payment_method' ) ) { ?>
		<div class ="rs_encash_payment_method">
			<p>
				<label><?php echo wp_kses_post( get_option( 'rs_encashing_payment_method_label' ) ); ?></label>
			</p>
			<p>
					...
					if ( 'yes' == $AllowToSavePaymentMethod && 'encash_through_custom_payment' == get_user_meta( get_current_user_id(), 'rs_cashback_previous_payment_method', true ) ) {
						?>
							...
							if ( check_whether_hoicker_is_active() ) {
								...
								?>
						<option value="<?php echo wp_kses_post( $WalletLabel ); ?>"><?php echo wp_kses_post( $WalletLabel ); ?></option>
					<?php } ?>
				</select>
			</p>
		</div>
	...
	if ( check_whether_hoicker_is_active() ) {
		...
		?>
		<input type="hidden" value="<?php echo wp_kses_post( $WalletLabel ); ?>" id="is_walletia_selected" class="is_walletia_selected" name="is_walletia_selected">
		<div class ="rs_encash_wallet">
			<p>
				<label><?php echo wp_kses_post( get_option( 'rs_encashing_wallet_label' ) ); ?></label>
			</p>
		</div>
		<?php
	}
	if ( '1' == get_option( 'rs_select_payment_method' ) || '3' == get_option( 'rs_select_payment_method' ) ) {
		...
		<div class ="rs_encash_paypal_address">
			<p>
				<label><?php echo wp_kses_post( get_option( 'rs_encashing_payment_paypal_label' ) ); ?></label>
			</p>
			<p>
				...
				if ( 'yes' == $AllowToSavePaymentMethod && '' != get_user_meta( get_current_user_id(), 'rs_paypal_payment_details', true ) ) {
					echo wp_kses_post( get_user_meta( get_current_user_id(), 'rs_paypal_payment_details', true ) );
				}
				?>
					   "/>
			</p>
		...
		<div class="error" id ="paypal_email_empty_error">
			<?php echo wp_kses_post( get_option( 'rs_error_message_paypal_email_empty' ) ); ?>
		</div>
		<div class="error" id ="paypal_email_format_error">
			<?php echo wp_kses_post( get_option( 'rs_error_message_paypal_email_wrong' ) ); ?>
		</div>
		<?php
	}
	if ( '2' == get_option( 'rs_select_payment_method' ) || '3' == get_option( 'rs_select_payment_method' ) ) {
		...
		<div class ="rs_encash_custom_payment_option_value">
			<p>
				<label><?php echo wp_kses_post( get_option( 'rs_encashing_payment_custom_label' ) ); ?></label>
			</p>
			<p>
				<textarea name ="rs_encash_custom_payment_option_value" id="rs_encash_custom_payment_option_value" rows= "3" cols= "50">
					...
					if ( 'yes' == $AllowToSavePaymentMethod && '' != get_user_meta( get_current_user_id(), 'rs_custom_payment_details', true ) ) {
						echo wp_kses_post( get_user_meta( get_current_user_id(), 'rs_custom_payment_details', true ) );
					}
					?>
				</textarea>
			</p>
		...
		<div class="error" id="paypal_custom_option_empty_error" style="display:none;">
			<?php echo wp_kses_post( get_option( 'rs_error_custom_payment_field_empty' ) ); ?>
		</div>
		<?php
	}
	if ( 'yes' == get_option( 'rs_enable_recaptcha_to_display' ) && '' != get_option( 'rs_google_recaptcha_site_key' ) ) {
		...
		<div class="rs_enable_recaptcha_to_display">
			<p>
				<label>
					<?php echo wp_kses_post( get_option( 'rs_google_recaptcha_label' ) ); ?>
				</label>
			</p>
			<p>
		...
		<div class="error" id="recaptcha_empty_error">
			<?php echo wp_kses_post( get_option( 'rs_error_recaptcha_field_empty' ) ); ?>
		</div>
		<?php
	}
	?>
	<div class ="rs_encash_submit">
		<input type="submit" value="<?php echo wp_kses_post( get_option( 'rs_encashing_submit_button_label' ) ); ?>" id="submit_cashback"/>
	</div>
	<div class="success_info" id ="encash_form_success_info">
		<b><?php echo wp_kses_post( get_option( 'rs_message_encashing_request_submitted' ) ); ?></b>
	</div>
</form>
<?php
