...
if ( ! class_exists( 'RS_Imp_Exp_Module_Export_Points' ) ) {
	...
	class RS_Imp_Exp_Module_Export_Points extends SRP_Action_Scheduler {
		...
		public function get_redirect_url() {
			return add_query_arg( array ( 'page' => 'rewardsystem_callback', 'tab' => 'fprsmodules', 'section' => 'fpimportexport', 'export_points' => 'yes' ), SRP_ADMIN_URL );
		}
		...
		public function chunked_scheduler_action( $user_ids ) {
			...
			foreach ( $user_ids as $user_id ) {
				...
				foreach ( $points_logs as $points_value ) {
					...
					$data_to_merge[] = array (
						'user_name' => $user_name,
						'points'    => round_off_type( $overall_points ),
						'date'      => $expiry_date,
					);
				}
			}
