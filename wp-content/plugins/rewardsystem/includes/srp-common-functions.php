...
include_once( 'srp-layout-functions.php' ) ;
include_once( 'srp-template-functions.php' ) ;
include_once( 'srp-post-functions.php' ) ;
include_once( 'srp-admin-functions.php') ;
include_once( 'frontend/srp-frontend-functions.php') ;
...
if ( ! function_exists( 'rs_send_mail_for_actions' ) ) {
	...
	function rs_send_mail_for_actions( $to, $event_slug, $earned_point, $user_name = '', $order_id = '' ) {
		...
		if ( 'yes' == get_option( 'rs_send_mail_for_bday_points' ) ) {
			if ( 'BRP' == $event_slug ) {
				$subject = get_option( 'rs_email_subject_for_bday_points' ) ;
...
if ( ! function_exists( 'send_mail_for_product_purchase' ) ) {
	...
	function send_mail_for_product_purchase( $user_id, $order_id, $email_type = 'earning' ) {
		...
		foreach ( $templates as $emails ) {
			...
			$SendMail = ( '1' == $emails->mailsendingoptions ) ? ( '1' != get_post_meta( $order_id , 'rsearningtemplates' . $emails->id , true ) ) : true ;
			if ( $SendMail ) {
...
if ( ! function_exists( 'block_points_for_renewal_order_sumo_subscriptions' ) ) {
	...
	function block_points_for_renewal_order_sumo_subscriptions( $order_id, $enable ) {
		...
		if ( function_exists('sumosubs_is_renewal_order') && sumosubs_is_renewal_order($order) &&  'yes' == get_post_meta( $order_id , 'sumo_is_subscription_order' , true )) {
			return false ;
		}
...
if ( ! function_exists( 'expiry_date_for_points' ) ) {
	...
	function expiry_date_for_points() {
		$date = 999999999999 ;
		if ( 'yes' == get_option( 'rs_point_expiry_activated' ) && ! empty( get_option( 'rs_point_to_be_expire' ) ) ) {
...
if ( ! function_exists( 'round_off_type_for_currency' ) ) {
	...
	function round_off_type_for_currency( $currency, $args = array() ) {
		if ( '1' == get_option( 'rs_round_off_type' ) ) {
			...
			if ( '1' == get_option( 'rs_roundoff_type_for_currency' ) ) {
...
if ( ! function_exists( 'update_order_meta_if_points_awarded' ) ) {
	...
	function update_order_meta_if_points_awarded( $orderid, $userid ) {
		...
		update_post_meta( $orderid , 'reward_points_awarded' , 'yes' ) ;
		update_post_meta( $orderid , 'earning_point_once' , 1 ) ;
...
if ( ! function_exists( 'get_list_of_modules' ) ) {
	...
	function get_list_of_modules( $value = '' ) {
		return array(
			...
			'fpdiscounts'          => 'name' == $value ? __( 'SUMO Discounts Compatibility' , 'rewardsystem' ) : get_option( 'rs_discounts_compatability_activated' ) ,
			'fpcoupon'             => 'name' == $value ? __( 'SUMO Coupon Compatibility' , 'rewardsystem' ) : get_option( 'rs_coupon_compatability_activated' ) ,
...
if ( ! function_exists( 'rs_custom_search_fields' ) ) {
	...
	function rs_custom_search_fields( $args ) {
		...
		if ( ( float ) WC_VERSION <= ( float ) ( '2.2' ) ) {
			?>
			<select <?php echo esc_attr( $args[ 'multiple' ] ? 'multiple="multiple"' : ''  ) ; ?> name="
			<?php
			echo esc_attr( '' !== $args[ 'name' ] ? $args[ 'name' ] : $args[ 'id' ]  ) ;
			if ( $args[ 'multiple' ] ) {
				?>
																									 []<?php } ?>" 
																								 id="<?php echo esc_attr( $args[ 'id' ] ) ; ?>" class="<?php echo esc_attr( $args[ 'id' ] ) ; ?>"
																									 ...
																									 <?php
																										if ( is_array( $args[ 'options' ] ) ) {
																											foreach ( $args[ 'options' ] as $id ) {
																												...
																												if ( $option_value ) {
																													?>
							<option value="<?php echo esc_attr( $id ) ; ?>" <?php echo wp_kses_post( $args[ 'selected' ] ? 'selected="selected"' : ''  ) ; ?>><?php echo esc_html( $option_value ) ; ?></option>
																													<?php
																												}
																											}
																										}
			...
			<?php
		} else if ( ( float ) WC_VERSION < ( float ) ( '3.0' ) ) {
			?>
			<input type="hidden" name="<?php echo esc_attr( '' !== $args[ 'name' ] ? $args[ 'name' ] : $args[ 'id' ]  ) ; ?>" id="<?php echo esc_attr( $args[ 'id' ] ) ; ?>" class="<?php echo esc_attr( $args[ 'class' ] ) ; ?>" data-action="<?php echo esc_attr( $args[ 'action' ] ) ; ?>" data-placeholder="<?php esc_html_e( esc_attr( $args[ 'placeholder' ] ) , 'rewardsystem' ) ; ?>" <?php echo esc_attr( $args[ 'multiple' ] ? 'data-multiple="true"' : ''  ) ; ?> <?php echo esc_html( $args[ 'allow_clear' ] ? 'data-allow_clear="true"' : ''  ) ; ?> 
				   <?php
					if ( $args[ 'selected' ] ) {
						?>
					   data-selected="
					   ...
					   " value="
						<?php
						echo wp_kses_post( implode( ',' , array_keys( $json_ids ) ) ) ;
					}
					?>
				   "/>
				   <?php
		...
		if ( '2' == $ProductsToApplyPointPrice ) {
			if ( '1' == get_option( 'rs_pricing_type_global_level' ) ) {
				if ( '' != get_option( 'rs_local_price_points_for_product' , '' ) && '1' == get_option( 'rs_global_point_price_type' , '1' ) ) {
					return '1' ;
				} else if ( '2' == get_option( 'rs_global_point_price_type' , '1' ) ) {
					return '1' ;
				}
			} else {
...
function is_product_level( $productid, $variationid, $item, $checklevel, $referred_user, $getting_referrer, $socialreward, $rewardfor, $payment_price, $itemquantity ) {
	...
	if ( ( 'yes' == $productlevel ) || ( '1' == $productlevel ) ) {
		...
		foreach ( $term as $terms ) {
			...
			if ( $getcount >= 1 ) {
				if ( ( 'yes' == $categorylevel ) ) {
					...
					} else if ( ( '2' == $categorylevelrewardtype ) && '' != $categorylevelrewardpercent ) {
						if ( 'yes' == $checklevel ) {
							$cat_level_enabled[] = '2' ;
						} else {
...
if ( ! function_exists( 'order_total_in_order_detail' ) ) {
	...
	function order_total_in_order_detail( $total, $order ) {
		...
		foreach ( $order->get_items()as $item ) {
			...
			if ( '' != $PointPriceData[ $ProductId ] && null == $CheckIfBundledProduct ) {
		...
		$TotalPoints   = round_off_type( $TotalPoints , array() , false ) + redeem_point_conversion( $shipping_tax , $user_id ) + redeem_point_conversion( $excl_tax_total , $user_id ) + redeem_point_conversion( $fee_total , $user_id ) ;
...
if ( ! function_exists( 'rs_get_first_purchase_point' ) ) {
	...
	function rs_get_first_purchase_point( $order = false ) {
		...
		if ( '1' == get_option( 'rs_global_reward_points_type' , 1 ) ) {
			// Fixed Amount.
			$points = ( float ) get_option( 'rs_reward_points_for_first_purchase_in_fixed' , '0' ) ;
		} else if ( '2' == get_option( 'rs_global_reward_points_type' ) ) {
			// Percentage of Cart Subtotal.
			if ( is_object( $order ) ) {
				$points = get_post_meta( $order->get_id() , 'rs_first_purchase_point_for_order' , true ) ;
			} else {
...
if ( ! function_exists( 'rs_validate_referral_system_restrictions' ) ) {
	...
	function rs_validate_referral_system_restrictions( $order ) {
		...
		$referrer_name = get_post_meta( $order->get_id() , '_referrer_name' , true ) ;
