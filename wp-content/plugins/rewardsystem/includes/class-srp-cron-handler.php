...
if (!class_exists('SRP_Cron_Handler')) {
	...
	class SRP_Cron_Handler {
		...
		public static function send_mail( $newdata, $userid, $Templates) {
			...
			$site_referral_url = 'yes' == get_option('rs_restrict_referral_points_for_same_ip') ? esc_url_raw(add_query_arg(array('ref' => $user->user_login, 'ip' => base64_encode(get_referrer_ip_address())), site_url())) : esc_url_raw(add_query_arg(array('ref' => $user->user_login), site_url()));
		...
		public static function award_product_purchase_points_based_on_cron( $order_id) {
			if ('yes' == get_post_meta($order_id, 'rs_order_status_reached', true)) {
		...
		public static function send_mail_based_on_cron() {
			...
			global $wpdb;
			$email_templates = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}rs_templates_email"); //all email templates
			...
			$matched_user_ids = get_users(array('include' => $matched_user_ids, 'fields' => 'ids'));
			if (!srp_check_is_array($matched_user_ids)) {
				return;
			}
						/**
			...
			foreach ($email_templates as $emails) {
				...
				if ('1' == $emails->mailsendingoptions) { //Send Mail Only Once
					...
					if ('1' == $emails->sendmail_options) { //Send Mail for All User
		...
		public static function available_points_mail_based_on_cron( $userid, $emails, $SiteUrl, $userpoint) {
			...
			$site_referral_url = 'yes' == get_option('rs_restrict_referral_points_for_same_ip') ? esc_url_raw(add_query_arg(array('ref' => $user->user_login, 'ip' => base64_encode(get_referrer_ip_address())), $referral_url)) : esc_url_raw(add_query_arg(array('ref' => $user->user_login), $referral_url));
