...
if ( ! class_exists( 'RSImportExport' ) ) {
	...
	class RSImportExport {
				...
				<table class="wp-list-table widefat fixed posts">
					...
					<tbody>
						...
						foreach ( get_option( 'rewardsystem_csv_array' )as $newcsv ) {
							...
							<tr>
								<td>
									<?php echo wp_kses_post(( isset( $newcsv[ 0 ] ) && ! empty( $newcsv[ 0 ] ) ) ? $newcsv[ 0 ] : '' ); ?>
								</td>
								<td>
									<?php echo wp_kses_post(( isset( $newcsv[ 1 ] ) && ! empty( $newcsv[ 1 ] ) ) ? $newcsv[ 1 ] : '0') ; ?>
								</td>
								<td>
									<?php
									if ( isset( $newcsv[ 2 ] ) ) {
		...
		public static function imp_user_points() {
			...
			if ( $file_error > 0 ) {
				echo wp_kses_post('Error: ' . $file_error . '<br>' );
			} else {
		...
		public static function inputCSV( $data_path ) {
			...
			while ( ( $data = fgetcsv( $handle , 1000 , ',' ) ) !== false ) {
				$datas        = 0 != $data[ 2 ] ? strtotime( $data[ 2 ] ) : '' ;
