...
<div class="rs-multiple-anniversary-points-table-content-popup-wrapper">
	<div class="rs-multiple-anniversary-details">
		<div>
			...
			<span><?php echo wp_kses_post( $user->user_login ); ?></span>
		</div>
		<br/>
		<div>
			<span><b><?php echo esc_html__( 'Email ID: ', 'rewardsystem' ); ?></b></span>
			<span><?php echo wp_kses_post( $user->user_email ); ?></span>
		</div>
		<div>
			<h3><?php echo esc_html__( 'Anniversary Details', 'rewardsystem' ); ?></h3>
			<?php include (SRP_PLUGIN_PATH . '/includes/admin/views/backbone-modal/html-anniversary-multiple-dates-table-content.php'); ?>
		</div>
	</div>
				...
				<tr>
					<td>
						...
						if ( isset( $multiple_anniv_value[ 'earneddate' ] ) ) :
							...
							echo wp_kses_post( isset( $stored_field_names[ $earned_date ] ) ? $stored_field_names[ $earned_date ] : '-'  );
						endif;
						?>
					</td>
