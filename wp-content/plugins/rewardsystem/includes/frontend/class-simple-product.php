...
if ( ! class_exists( 'RSFunctionforSimpleProduct' ) ) {
	...
	class RSFunctionforSimpleProduct {
		...
		public static function rs_msg_in_shop_page() {
			...
			?>
			<div class="woocommerce-info"><?php echo wp_kses_post($ShopPageMsg) ; ?></div>
			<?php
		}
			...
			if ( is_object( $ProductObj ) && ( 'simple' == srp_product_type( $post->ID ) || ( 'subscription' == srp_product_type( $post->ID ) ) ) ) {
				...
				if ( ! $sumo_bookings_check ) {
					...
					if ( is_shop() || is_product_category() ) {
						if ( is_user_logged_in() ) {
							...
							if ( '1' == get_option( 'rs_show_hide_buy_pont_message_for_simple_in_shop_guest' )  ) {
								$shop_msg = do_shortcode( get_option( 'rs_buy_point_message_in_shop_page_for_simple' ) ) ;
								return $price . '<br>' . $shop_msg ;
							}
						}
				...
				if ( is_page() || is_tax( 'pwb-brand' ) ) {
					if ( is_user_logged_in() ) {
						...
						if ( '1' == get_option( 'rs_show_hide_buy_point_message_for_simple_in_custom_shop_guest' ) ) {
							$shop_msg = do_shortcode( get_option( 'rs_buy_point_message_in_custom_shop_page_for_simple' ) ) ;
							return $price . '<br>' . $shop_msg ;
						}
					}
			...
			if ( is_object( $ProductObj ) && check_if_variable_product( $ProductObj ) ) {
				...
				if ( is_page() || is_tax( 'pwb-brand' ) ) {                                                                        //Shop and Category Page Message for Variable Product                            
					if ( 'yes' == get_option( 'rs_enable_display_earn_message_for_variation_custom_shop' ) ) {
						...
						if ( ! empty( $pointmin ) ) {
							return self::rs_function_to_get_msg_with_gift_icon_for_variable( '' , $price , $point_price_display ) ;
						}
					} else {
				...
				if ( is_shop() || is_product_category() || is_product_tag() || is_tax( 'dc_vendor_shop' ) ) {                                                                        //Shop and Category Page Message for Variable Product                            
					if ( 'yes' == get_option( 'rs_enable_display_earn_message_for_variation' ) && true == $bool ) {
						if (  is_user_logged_in() ) {                                                    
							...
							if ( '1' == get_option('rs_show_hide_message_for_variable_in_shop_guest' , '1')) {
								return self::rs_function_to_get_msg_with_gift_icon_for_variable( '' , $price , $point_price_display ) ;
							}
				...
				if ( $getshortcodevalues > 0 ) {
					...
					if ( is_shop() || is_product_category() || is_product_tag() || is_tax( 'dc_vendor_shop' ) ) {//Shop and Category Page Message for Simple Product
						...
						if ( ! $sumo_bookings_check ) {
							...
							if ( is_user_logged_in() ) {
								...
								if ( '1' == get_option( 'rs_show_hide_message_for_simple_in_shop_guest' ) && true == $bool ) {
					...
					if ( ( is_page() || is_tax( 'pwb-brand' ) ) && $bool ) {
						...
						if ( is_user_logged_in() ) {
							...
							//Shop Page Message for Simple Product for Guest
							if ( '1' == get_option( 'rs_show_hide_message_for_simple_in_custom_shop_guest' ) ) {
		...
		public static function rs_function_to_get_msg_with_gift_icon_for_variable( $earnmessage, $price, $point_price_display, $related_product = false ) {
			...
			if ( '1' == get_option( '_rs_enable_disable_gift_icon' )) {
				if ( '1' == get_option( 'rs_enable_disable_point_priceing' ) && 'yes' == get_option( 'rs_point_price_activated' ) && $VisibilityForPointPrice ) {
					if ( '' != get_option( 'rs_image_url_upload' ) ) {
						if (  '1' == get_option( 'rs_message_position_in_single_product_page_for_variable_products' ) ) {
							if ( 'no' == get_option( 'rs_enable_product_category_level_for_product_purchase' ) && '1' != get_option( 'rs_award_points_for_cart_or_product_total' ) ) {
				...
				if ( is_product() ) {
					...
					if ( '1' == get_option( 'rs_enable_disable_point_priceing' ) && 'yes' == get_option( 'rs_point_price_activated' ) && $VisibilityForPointPrice ) {
						if ( 'no' == get_option( 'rs_enable_product_category_level_for_product_purchase' ) && '1' != get_option( 'rs_award_points_for_cart_or_product_total' )) {
							...
							if ( '1' == get_option( 'rs_msg_position_for_var_products_in_shop_page' )) {
		...
		public static function display_purchase_msg_in_single_product_page() {
			...
			?>
			<div class="woocommerce-info rs_message_for_single_product"><?php echo wp_kses_post($replace) ; ?></div>
			<?php
		}
		...
		public static function display_cart_total_based_product_purchase_msg_in_single_product_page() {
			...
			?>
			<div class="woocommerce-info"><?php echo wp_kses_post($ProductPageMsg) ; ?></div>
			<?php
		}
		...
		public static function display_product_review_msg_in_single_product_page() {
			...
			?>
			<div class="woocommerce-info rs_message_for_single_product"><?php echo wp_kses_post($ProductReviewMsginProductPage) ; ?></div>
			<?php
		}
			...
			if ( is_user_logged_in() ) {
				...
				if ( referral_points_for_simple_product() > 0 ) {
					if ( '1' == get_option( 'rs_show_hide_message_for_single_product_referral' ) && 'yes' == get_option( 'rs_referral_activated' ) ) {
						...
						<div class="woocommerce-info rs_message_for_single_product">
							...
							echo wp_kses_post(do_shortcode( $strrplc )) ;
							?>
						</div>
						<?php
					}
				...
				if ( 'yes' == get_option( 'rs_buyingpoints_activated' ) ) {
					...
					if ( '1' == get_option( 'rs_show_hide_buy_point_message_for_single_product' ) && 'yes' == get_post_meta( $post->ID , '_rewardsystem_buying_reward_points' , true ) && $buy_points > 0 ) {
						...
						<div class="woocommerce-info rs_message_for_single_product"><?php echo wp_kses_post(do_shortcode( get_option( 'rs_buy_point_message_for_single_product_point_rule' )) ) ; ?></div>
						<?php
					}
				}
				...
				if ( is_object( $checkproducttype ) && ( 'simple' == srp_product_type( $post->ID ) || 'subscription' == srp_product_type( $post->ID ) || 'bundle' == srp_product_type( $post->ID ) ) ) {
					...
					if ( $rewardpoints > 0 && ! $sumo_bookings_check ) {
						...
						?>
						<div class="woocommerce-info rs_message_for_single_product"><?php echo wp_kses_post(do_shortcode( $message ) ); ?></div>
						<?php 
					} elseif ( $rewardpoints > 0 && $sumo_bookings_check ) {
						?>
						<div class="woocommerce-info rs_message_for_single_product"><?php echo wp_kses_post(get_option( 'rs_message_for_booking_product' )) ; ?></div>
				...
				if ( isset( $_COOKIE[ 'rsreferredusername' ] ) ) {
					if ( 'yes' == get_option( 'rs_referrer_earn_point_purchase_by_guest_users' ) && '1' == get_option( 'rs_show_hide_message_for_single_product_guest_referral' )  ) {
						...
						if ( $referralpoints > 0 ) {
							...
							?>
							<div class="woocommerce-info rs_message_for_single_product"><?php echo wp_kses_post(do_shortcode( $msg )) ; ?></div>
							<?php
						}
					}
				}
		...
		public static function display_minimum_quantity_error_message_simple_product() {
			...
			?>
			<div class="woocommerce-error rs-minimum-quantity-error-simple"><?php echo wp_kses_post($message); ?></div>
			<?php
		}
