	...
	class FP_Rewardsystem_Frontend_Ajax {
		...
		public static function generate_referral_link() {
			...
			if ( ! isset( $_POST ) || ! isset( $_POST[ 'url' ] ) || empty($_POST[ 'url' ] ) ) {
				throw new exception( __( 'Invalid URL' , 'rewardsystem' ) ) ;
			}
			...
			try {
				...
				$RefURL      = ( 'yes' == get_option( 'rs_restrict_referral_points_for_same_ip' ) ) ? add_query_arg( array( 'ref' => $KeyforQuery , 'ip' => base64_encode( get_referrer_ip_address() ) ) , esc_url_raw($_POST[ 'url' ]) ) : add_query_arg( array( 'ref' => $KeyforQuery ) , esc_url_raw($_POST[ 'url' ]) ) ;
		...
		public static function mail_referral_link_to_friends() {
			...
			try {
				...
				if ( ! srp_check_is_array( $Email ) ) {
					throw new Exception( __( 'There is no Email-Id' , 'rewardsystem' ) ) ;
				}
		...
		public static function cashback_request() {
			...
			try {
				...
				if ( 'yes' == get_option( 'rs_email_notification_for_Admin_cashback' ) ) {
					...
					} else if ( 'encash_through_custom_payment' == $PaymentMethod ) {
		...
		public static function redeem_gift_voucher() {
			...
			if ( ! isset( $_POST ) ) {
				throw new exception( __( 'Invalid Code' , 'rewardsystem' ) ) ;
			}
			...
			if ( ! empty( $CodeUsage ) ) {
				if ( '1' == $CodeUsage ) {
					if ( ! srp_check_is_array( $VoucherUsedBy ) ) {
						...
						$wpdb->update( "{$wpdb->prefix}rsgiftvoucher" , array( 'points' => $VoucherPoints , 'vouchercode' => $VoucherCode , 'vouchercreated' => $CreatedVoucher , 'voucherexpiry' => $ExpDate , 'memberused' => $UsedBy ) , array( 'id' => $VoucherData[ 0 ][ 'id' ] ) ) ;
						return $Content ;
					} else {
						throw new exception( addslashes( get_option( 'rs_voucher_code_used_error_message' ) ) ) ;
					}
					...
					if ( '1' == $UsageLimit && ! empty( $UsageLimitValue ) ) {
						if ( $UsageCount < $UsageLimitValue ) {
							...
							$wpdb->update( "{$wpdb->prefix}rsgiftvoucher" , array( 'points' => $VoucherPoints , 'vouchercode' => $VoucherCode , 'vouchercreated' => $CreatedVoucher , 'voucherexpiry' => $ExpDate , 'memberused' => $SerializedData ) , array( 'id' => $VoucherData[ 0 ][ 'id' ] ) ) ;
							return $Content ;
						} else {
							throw new exception( addslashes( get_option( 'rs_voucher_code_used_error_message' ) ) ) ;
						}
		...
		public static function unsunscribe_or_subscribe_mail() {
			...
			try {
				...
				$Content = 'yes' == wc_clean(wp_unslash($_POST[ 'subscribe' ])) ? __( 'Successfully Unsubscribed...' , 'rewardsystem' ) : __( 'Successfully Subscribed...' , 'rewardsystem' ) ;
				wp_send_json_success( array( 'content' => $Content ) ) ;
			} catch ( Exception $ex ) {
				wp_send_json_error( array( 'error' => $ex->getMessage() ) ) ;
			}
		...
		public static function save_nominee() {
			...
			try {
				...
				$Content = __( 'Nominee Saved' , 'rewardsystem' ) ;
				wp_send_json_success( array( 'content' => $Content ) ) ;
			} catch ( Exception $ex ) {
				wp_send_json_error( array( 'error' => $ex->getMessage() ) ) ;
			}
		...
		public static function reward_gateway_msg() {
			...
			try {
				...
				wp_send_json_success( array( 'rewardpoints' => $GatewayPoints , 'title' => sanitize_text_field($_POST[ 'gatewaytitle' ]) , 'restrictedmsg' => $MsgToDisplay , 'earn_gateway_message' => $gateway_reward_message ) ) ;
			} catch ( Exception $ex ) {
				wp_send_json_error( array( 'error' => $ex->getMessage() ) ) ;
			}
		}
