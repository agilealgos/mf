...
if (!class_exists('Bonus_Points_Handler')) {
	...
	class Bonus_Points_Handler {
		...
		public static function may_be_add_bonus_for_orders_placed_with_repeat( $order_id) {
			...
			if ('yes' == get_post_meta($order_id, 'rs_bonus_for_orders_with_repeat_awarded_once', true)) {
				return;
			}
			...
			update_post_meta($order_id, 'rs_bonus_points_for_orders_with_repeat', $bonus_points);
			// Update meta to check occur once. 
			update_post_meta($order_id, 'rs_bonus_for_orders_with_repeat_awarded_once', 'yes');
		...
		public static function may_be_add_bonus_for_orders_placed_without_repeat( $order_id) {
			...
			if ('yes' == get_post_meta($order_id, 'rs_bonus_for_orders_without_repeat_awarded_once', true)) {
				return;
			}
			...
			// Update bonus point meta. 
			update_post_meta($order_id, 'rs_bonus_points_for_orders_without_repeat', $bonus_points);
			// Update order count meta. 
			update_post_meta($order_id, 'rs_orders_count_without_repeat', array_sum($order_counts));
			// Update meta to check occur once. 
			update_post_meta($order_id, 'rs_bonus_for_orders_without_repeat_awarded_once', 'yes');
		}
		...
		public static function save_order_meta_number_of_orders_type( $sliced_order_id) {
			...
			if ('yes' == get_post_meta($sliced_order_id, 'rs_recorded_order_no_of_orders_type', true)) {
				return 0;
			}

			update_post_meta($sliced_order_id, 'rs_recorded_order_no_of_orders_type', 'yes');

			if (isset(self::$current_order_id)) {
				update_post_meta($sliced_order_id, 'rs_bonus_awarded_order_id', self::$current_order_id);
			}
